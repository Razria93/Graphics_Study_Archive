...
- 되돌리기 어려운 작업은 실행 전 범위와 위험도를 보고하고 승인받는다.
- repository 존속에 영향을 줄 수 있는 위험을 발견하면 작업을 멈추고 먼저 보고한다.
- 조사와 비교는 read-only 범위에서 병렬화할 수 있다.
- 파일 수정, stage, commit과 GitHub remote 변경은 하나의 실행 주체가 직렬로 수행한다.
- 하위 agent가 조사한 결과는 메인 agent가 실제 파일과 상태를 다시 확인한다.
