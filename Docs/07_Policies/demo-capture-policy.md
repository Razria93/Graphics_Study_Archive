...
Demo 계획과 evidence 설명의 정본은 `Docs/04_Demos`에 둔다. 검토된 evidence 파일은 `Docs/_assets`에 둔다.

`Docs/_assets`는 raw capture 저장소가 아니다. `Docs/04_Demos` 또는 `Docs/03_Verification`에서 실제로 참조할 검증 완료 evidence만 둔다.
...
- `Docs/04_Demos/*/demo-index.md` 또는 `Docs/04_Demos/capture-plan.md`에서 참조할 계획이 있다.
- `Docs/03_Verification/capture-matrix.md`에 capture 상태를 기록할 수 있다.
- public 후보 여부 또는 private-only 사유를 `Docs/06_Publication`에서 판단할 수 있다.
