...
검증 결과와 확인 시점은 분리한다. 문서화 이전에 직접 확인한 성공은 결과를
`성공`, provenance를 `과거 확인`, 현재 재검증을 `필요`로 기록한다.
표에서는 `현재 재검증 | 필요` 형식을 사용하고, 본문에서는 이를
`재검증 필요`라고 표현할 수 있다. 상세 기준은 [Verification Policy](../06_Policies/verification-policy.md)를 따른다.
