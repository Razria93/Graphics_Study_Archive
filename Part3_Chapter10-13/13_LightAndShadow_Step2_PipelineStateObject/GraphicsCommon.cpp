...
// extern의 경우, 이와 같이 cpp에서 변수를 다시한번 정의해주어야함
// extern :
// - 외부에서 정의된 변수나 함수를 참조할 때 사용하는 키워드
// - extern을 사용하면 해당 변수가 다른 파일이나 범위(외부)에서 정의되어 있다라고 알려주는 것
// - 해당 내용들이 class가 아닌 namespace 이기 때문에 extern으로 운용함
// - 교수님의 경우 클래스 + 싱글턴으로 운용했을 것이라 이야기함.
...
void Graphics::InitSamplers(ComPtr<ID3D11Device>& device)
{
	...
	// 즉 쉐이더에서 샘플러를 참조하는 순서와 동일해야한다는 것
	// 쉐이더는 바이트단위로 데이터를 넘겨받아 재구성하는 것이 때문에 CPU에서 어떠한 구성으로 만들어지는지 모름.
	// ex.
...
void Graphics::InitPipelineStates(ComPtr<ID3D11Device>& device)
{
	...
	// skyboxSolidPSO
	// skybox를 클래스로 별도로 두는 것이 아니라 똑같은 Mesh로 취급하되 이후 처리(쉐이더)를 별도로 하겠다는 것
