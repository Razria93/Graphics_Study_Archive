...
| Chapter03 순차 작업 | 진행 중 | Step4 DrawingSphere부터 Step13 Transparency까지 build/run, README·Topic과 독립 상세 Demo를 완료했다. Step10~12와 Step13 ground는 사용자 직접 생성 석재 texture를 사용하고 Step13 background는 사용자 직접 생성 해수면·하늘 texture를 사용한다. |
...
1. `Part1_Chapter03` Step14 CubeEnvironment를 build/run하고 문서화한다.
2. Step14 environment asset의 attribution과 사용 subset을 판정한다.
