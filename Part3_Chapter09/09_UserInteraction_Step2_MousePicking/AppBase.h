...
class AppBase {
  ...
  public:
    ...
    ComPtr<ID3D11Texture2D> m_indexTexture;        // RTV 대상 텍스쳐
    ComPtr<ID3D11Texture2D> m_indexTempTexture;    // ShaderResourceView 만 만들 수 있는 Picking용 렌더링 결과물을 복사한 Texture2D
    ComPtr<ID3D11Texture2D> m_indexStagingTexture; // 1x1 작은 크기의 CPU Read가능한 Texture
    ComPtr<ID3D11RenderTargetView> m_indexRenderTargetView; // RTV
