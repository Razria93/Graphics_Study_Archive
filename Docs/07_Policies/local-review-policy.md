...
- local 상세 조사에서 재사용 가능한 결론만 `Docs/05_WorkLogs/reviews/`에 요약한다.
...
| tracked 요약 후보 | `local/study-review/` -> `Docs/05_WorkLogs/reviews/` | 반복 조사 방지를 위한 결론만 요약 |
...
`Docs/05_WorkLogs/reviews/`로 옮길 수 있는 내용은 아래로 제한한다.
