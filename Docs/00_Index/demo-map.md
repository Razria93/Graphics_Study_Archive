...
이 문서는 capture와 video의 계획, 위치, 공개 판단 연결을 안내한다.
...
| screenshot 계획 | `Docs/03_Demos/capture-plan.md` | screenshot 확보 기준 |
...
| 등급 | 기준 | 처리 |
| --- | --- | --- |
| 최소 capture | 예제 실행 여부를 보여주는 기본 capture/result | 주요 Work Unit에 최소 1개 확보 |
| 대표 capture | 포트폴리오에서 시각적 임팩트가 있는 capture/result | README, Issue, PR 후보로 연결 |
| 선택 capture | 보조 설명용 capture/result | 시간이 남을 때 정리 |
| video | 조작, animation, simulation처럼 정지 화면만으로 부족한 capture/result | 필요 항목만 선별 |
| 제외 | 권리, 출처, 품질 문제가 있는 capture/result | public 후보에서 제외 |
