...
void Mesh::InitCircle_Normalized(const float &normalizedRadius,
    ...
    for (float theta = 0.0f; theta < kTwoPi; theta += deltaTheta) {
        this->vertices.push_back(center + vec3(cos(theta), sin(theta), 0.0f) *
                                              normalizedRadius);
        this->colors.push_back(color);
    }
...
void Mesh::InitSquare_Normalized(const float &normalizedWidth,
                                 const float &normalizedHeight,
                                 const vec3 &color) {
    const auto scale =
        vec3(normalizedWidth * 0.5f, normalizedHeight * 0.5f, 1.0f);
