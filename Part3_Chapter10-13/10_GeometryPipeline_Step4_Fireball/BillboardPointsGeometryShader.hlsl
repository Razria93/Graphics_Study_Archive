                              ...
                              inout TriangleStream<PixelShaderInput> outputStream)
{
    ...
    float4 up = float4(0.0, 1.0, 0.0, 0.0);                 // up 방향은 고정
    float4 front = float4(eyeWorld, 1.0) - input[0].pos;    // front는 input 된 pos에서 eye방향으로 
    front.w = 0.0;                                          // 벡터이기 때문에 동차 0.0
    ...
    output.center = input[0].pos; // VertexShader에서 넘어온 pos값이 빌보드의 중심 (아직 변환 안함)
