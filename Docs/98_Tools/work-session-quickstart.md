...
모든 작업은 [Work Profile Policy](../06_Policies/work-profile-policy.md)에 따라 다음 두 profile 중 하나로 분류한다. Archive, GraphicsLab Development와 Terminal Maintenance 같은 별도 작업 유형은 만들지 않는다.

| Work type | 선택 기준 | 주요 산출물 |
| --- | --- | --- |
| `Graphics` | Graphics 기능을 구현·재구성·확장하거나 기존 구현을 검증하고 설명 가능한 결과로 정리함 | Implementation, Structure, Topic, Verification, Demo, README, WorkLog, PR body, Index, Progress |
| `Maintenance` | Policy, workflow, README, template, validator, 문서 구조, 링크·명칭·상태 또는 build 기반을 유지·보완함 | Maintenance change, Rationale, Impact, Regression verification, Maintenance PR body와 조건부 WorkLog·Index·Progress |

작업 시작 전에 [Work Contract](templates/work-contract.md)로 다음을 확정한다.
...
문서 감사 keyword와 세부 구성은 [Document System Audit Policy](../06_Policies/document-system-audit-policy.md)와 [Document System Audit Guide](document-system-audit-guide.md)를 따른다. Graphics Chapter 시작 기준선은 [Local Chapter Start Checkpoint](templates/local-chapter-start-checkpoint.md)를 사용한다.
...
대상 폴더의 README, AGENTS, 관련 policy와 template을 확인한 뒤 최소 범위로 수정한다. 선택한 Work Profile의 검증을 기본으로 하고 실제 변경 영향에 필요한 항목만 추가한다.
