...
{
	// 엔진의 스키닝 시스템은 boneIndex 기반
	// 즉, 이름 기반 데이터 → 인덱스 기반 데이터로 변환해야 함
	// 해당함수는 이 변환을 수행하는 것

	// 애니메이션의 클립 갯수만큼 배열 미리 확보
	m_aniData.clips.resize(pScene->mNumAnimations);

	// 애니메이션 클립별로 순회
	for (uint32_t i = 0; i < pScene->mNumAnimations; i++)
	{
		// m_aniData.clips의 i번째 배열 주소를 clip로 참조
		auto& clip = m_aniData.clips[i];

		// fbx에 저장되어 있는 실제데이터
		const aiAnimation* ani = pScene->mAnimations[i];

		// fbx의 기본 데이터를 엔진에 있는 clip에 저장
		clip.duration = ani->mDuration;
		clip.ticksPerSec = ani->mTicksPerSecond;

		// Key(애니메이션 데이터를 저장하는 구조체)를 boneNameToId의 키값쌍의 갯수만큼 배열을 확보
		// boneNameToId이 실질적으로 사용되는 bone의 갯수만큼 배열을 가지고 있기 때문
		...
		for (uint32_t c = 0; c < ani->mNumChannels; c++)
		{
			...
			//		- Rotation = keys
			//		- Scaling  = keys
			// 의 형태로 저장되어 있기 때문에 nodeAnim->mNodeName으로 Idx를 찾고 clip.key는 idx를 기반으로 저장함
			const aiNodeAnim* nodeAnim = ani->mChannels[c];
			const int boneId =
			    m_aniData.boneNameToId[nodeAnim->mNodeName.C_Str()];
			clip.keys[boneId].resize(nodeAnim->mNumPositionKeys); // key[idx] 내부의 frame에 대한 배열을 nodeAnim->mNumPositionKeys 만큼 확보 (frame의 갯수와 mNumPositionKeys의 갯수가 같음)
                       ...
                       bool revertNormals)
{
	...
	if (pScene)
	{
		...
		//	- fbx에 저장된 bone 구조가 Vertex에 영향을 주는 뼈도 있고 안주는 뼈도 있음
		//	- 아래 함수는 Vertex에 영향을 주는 뼈의 목록을 만드는 함수
		//	- fbx의 Vertex에 영향을 주는 bone의 '이름'이 저장되어 있음
		FindDeformingBones(pScene);

		// 2. 트리 구조를 따라 업데이트 순서대로 뼈들의 인덱스를 결정한다
		//	- 스켈레톤 node 트리를 Root → Child 순서로 순회하면서 boneNameToId[name] 에 실제 boneIndex를 채움.
		//	- fbx 파일에 내부적으로 트리구조로 저장되어 있음
		//	- 따라서 boneNameToId의 value값인 bone ID도 Root부터 0번으로 시작해서 부모 -> 자식 순으로 저장함
		//	- 어떤 idx는 부모의 idx보다 더 크면 됨
		//	- Tree Travels
		int counter = 0;
		UpdateBoneIDs(pScene->mRootNode, &counter);

		// 3. 업데이트 순서대로 뼈 이름 저장 (boneIdToName)
		//	- 인덱스 순서대로 업데이트를 하더라도 자연스럽게 부모의 transformation이 미리 결정되어 있도록 하기 위해서 자료구조를 저장할 때 트리구조를 따라서 저장함
		//	- 이를 정렬된 ID를 key로 한 boneIdToName에 다시 저장하는 것
		m_aniData.boneIdToName.resize(m_aniData.boneNameToId.size());
		for (auto& i : m_aniData.boneNameToId) // map
		...
		// 위는 사용되는 뼈와 index를 매핑한 map을 만드는 작업 (읽어들일 때 필요한 map구조 제작 [사전작업])

		// 실제로 저장 작업은 ProcessNode
		...
		// 애니메이션 정보 읽기
		// keyframe 애니메이션을 읽는 부분 (각 애니메이션을 구성하는 frame 단위의 변환데이터를 가져옴)
...
{
	// aiScene			  → 전체 모델
	// aiScene::mMeshes[] → 캐릭터를 이루는 여러 개의 메시 파츠들
	...
	for (uint32_t i = 0; i < scene->mNumMeshes; i++)
	{
		...
		if (mesh->HasBones())
		{
			for (uint32_t i = 0; i < mesh->mNumBones; i++)
			{
				...
				// boneNameToId 는 std::map
				//  - key가 없으면		→ 새로운 key/value 쌍을 map에 자동 생성
				//  - key가 이미 있으면 → value만 덮어쓰기(업데이트)
				//	- 따라서 위 코드는 mesh에 등록되어 있는 본들의 이름을 수집하여,
				//	  m_aniData.boneNameToId 에 key로 추가하고 value는 임시로 -1로 등록하는 것
...
void ModelLoader::ProcessNode(aiNode* node, const aiScene* scene, Matrix tr)
{
	...
	// 사용되는 각뼈의 부모 뼈를 찾아서 boneParents에 부모의 인덱스 저장 (boneId를 key값으로 저장되어 있어 후에 부모가 누군지 찾을 때 상ㅇ)
                                        ...
                                        aiTextureType type)
{
	...
	if (material->GetTextureCount(type) > 0)
	{
		...
		string fileName = string(filesystem::path(filepath.C_Str()).filename().string());
		...
		if (!filesystem::exists(fullPath))
		{
			...
			if (texture) // Embedded texture가 존재할 경우 -> 외부에 PNG로 저장함
			{
				// 원래는 fbx 따로 texture 따로 저장하는 것이 일반적이나 fbx 자체에 textureImage가 포함된 경우가 있음 / glpf : 바이너리 배열로 fbx 자체에 포함되어 있는 경우가 있음
...
MeshData ModelLoader::ProcessMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	// 스키닝 가능한 메시일 때만 아래 처리 수행
	//	- 메시의 각 버텍스가 하나 이상 Bone에 대한 Index + Weight를 갖고 있어서 애니메이션 시 해당 Bone 변환을 조합해 Vertex가 변형될 수 있다는 뜻
	//	- 스키닝이 불가능한 매시를 StaticMesh라고 하며 애니메이션이 불가능하고 움직여도 전체 Mesh에 대한 Transform만 가능
	if (mesh->HasBones())
	{
		// boneWeights / boneIndices를 저장할 컨테이너 선언 (vertex의 갯수만큼)
		// 하나의 vertex는 여러 bone의 영향을 받을 수 있음(최대 4개가 일반적)
		// 그래서 vertex마다 가중치 리스트가 필요 → vector<vector<T>> 구조
		vector<vector<float>> boneWeights(vertices.size());
		vector<vector<uint8_t>> boneIndices(vertices.size()); // vertex마다 [영향을 받는 vertex의 idx]를 vector<uint8_t>로 저장함 (최대 4개). 그걸 vertex마다 구현하면 vector<vector<uint8_t>>가 됨

		// 둘 다 모든 Bone에 대한 변환행렬 배열이며, 딱 한프레임(현재프레임)의 데이터만 저장함
		//	- 원래 Mesh(모델)의 로컬 좌표계에 있던 Vertex 위치를, 해당 Bone의 로컬 좌표계 기준으로 변환해주는 행렬
		//	- MeshSpace = 모델링 툴에서 Export된 바인드 포즈(T-포즈)의 좌표계
		m_aniData.offsetMatrices.resize(m_aniData.boneNameToId.size()); // 바인드 포즈 제거 행렬 (고정값)
		m_aniData.boneTransforms.resize(m_aniData.boneNameToId.size()); // 현재 애니메이션에서의 본 변환 (매 프레임 갱신)
		...
		for (uint32_t i = 0; i < mesh->mNumBones; i++)
		{
			// mesh에 저장된 mBone에 대하여 id로 컨버트하고 조회
			const aiBone* bone = mesh->mBones[i];
			...
			// offsetMatrices :
			//  - T-포즈에서 vertex를 Bone 기준으로 옮기는 역변환
			//  - Transpose : 행렬 메모리배치 변환
			m_aniData.offsetMatrices[boneId] =
			    Matrix((float*)&bone->mOffsetMatrix).Transpose();

			// 본마다 영향을 주는 Vertex를 for문으로 저장하는 부분
			// boneIndices, boneWeights는 mVertexId 기준으로 저장하기 때문에 저장은 Bone을 순회하며 저장하나 조회는 VertexID로 조회가능
			// mNumWeights : 가중치의 갯수 == bone에 관련된 Vertex의 갯수
		...
		// Vertices 정보를 skinnedVertices로 옮겨서 저장
		...
		for (int i = 0; i < vertices.size(); i++)
		{
			...
			// bone의 가중치와 idx를 저장하고 있던 것을 skinnedVertices로 옮겨서 저장
			// i : vertex idx
			// j : vertex에 영향을 주는 bone의 weight / vertex에 영향을 주는 bone의 idx
