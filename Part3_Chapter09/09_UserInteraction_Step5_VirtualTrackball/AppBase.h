...
class AppBase {
  ...
  public:
    ...
    bool m_leftButton = false;      // left의 down에 true, up에 false
    bool m_dragStartFlag = false;   // left가 down이 될 때마다 해당 flag는 true가 됨. 그러나 down 시점에만 true가 되고 ExampleApp에서 true가 되는 순간 if문으로 진입하자마자 false 로 바뀌기 때문에 한번 누르면 다시 누를때까지 false 상태 
