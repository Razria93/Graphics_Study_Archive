	...
	if (m_leftButton)
	{
		...
		if (m_selected)
		{
			...
			{ // 드래그를 시작하는 경우
				...
				// Near-Far 내에서 hit된 point까지의 거리인 dist의 상대적 위치
				// Ratio는 드래그 끝나기 전까지는 유지함
				prevRatio = dist / (cursorWorldFar - cursorWorldNear).Length();
				prevPos = pickPoint;
			}
			else
			{
				Vector3 curPos = cursorWorldNear +
				                 (cursorWorldFar - cursorWorldNear) * prevRatio;
				// dragTranslation = Vector3((curPos - prevPos).x, (curPos - prevPos).y, 0.0f);
				
				dragTranslation = curPos - prevPos;
