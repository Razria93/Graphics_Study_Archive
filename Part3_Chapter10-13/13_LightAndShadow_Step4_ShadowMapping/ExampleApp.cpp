...
void ExampleApp::UpdateLights(float dt)
{
	...
	for (int i = 0; i < MAX_LIGHTS; i++)
	{
		...
		if (light.type & LIGHT_SHADOW)
		{
			...
			// 여기 중요. light 기준으로 전용 GlobalCosnt 구성
...
void ExampleApp::Render()
{
	...
	// 그림자맵 만들기
	// 
	// HERE : 여기 중요. RSSetViewports를 기반으로 Viewport 바꾸는 함수를 구성함
	//		  SetMainViewport(), SetShadowViewport()
	AppBase::SetShadowViewport(); 
	AppBase::SetPipelineState(Graphics::depthOnlyPSO);
	for (int i = 0; i < MAX_LIGHTS; i++)
	{
		// IBL은 그림자를 그리지 않음
		// 대신 전체 일정한 방향으로 균일하게 작용하는 Directional Light를 사용함

		// if문에 사용된 문법은 bitFlag를 기반으로한 bitMask 체크방법
		// ConstantBuffer에 작성한 것처럼, 매크로를 이용하여 각 비트의 유효값에 의미를 부여함
		// 그리고 bitMask의 경우 비트 AND 연산인 & 를 이용하여 해당 요소가 활상화 되어 있는지를 확인 가능함

		// 해당 요소가 활성화가 되어있을 경우, type의 bitMask 또한 해당 자리에 1이 기록되어 있고,
		// 매크로로 지정된 값 또한 해당 bit 위치에 1이 기록되어 있기 때문에 결과적으로 & 는 1이 나옴

		// Shadow 사용 여부
		// LIGHT_SHADOW bit가 Masking 되어 있을경우 if문 실행함
		if (m_globalConstsCPU.lights[i].type & LIGHT_SHADOW)
		{
			// RTS 생략하고 DSV에만 렌더링 (컴파일러 경고 있을 수 있음)
			...
			// HERE : 중요! 
			// UpdateLights() 에서 각 Light에 대하여 light를 기준으로 GlobalConst를 재구성하고 있음
			// 이를 통해 light 시점에서 Depth값을 기록하는 것
