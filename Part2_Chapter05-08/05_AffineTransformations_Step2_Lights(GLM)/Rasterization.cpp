...
void Rasterization::Render(vector<vec4> &pixels) {
    ...
    for (const auto &mesh : this->meshes) {
        ...
        const mat4 scaleMatrix = glm::scale(transform.scale);
        const mat4 rotationMatrix =
            glm::rotate(transform.rotationZ, vec3(0.0f, 0.0f, 1.0f)) *
            glm::rotate(transform.rotationY, vec3(0.0f, 1.0f, 0.0f)) *
            glm::rotate(transform.rotationX, vec3(1.0f, 0.0f, 0.0f));
        const mat4 translationMatrix = glm::translate(transform.translation);
