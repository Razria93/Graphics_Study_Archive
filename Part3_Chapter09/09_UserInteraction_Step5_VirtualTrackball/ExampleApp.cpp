...
void ExampleApp::Update(float dt)
{
	...
	if (m_leftButton)
	{
		...
		// 커서의 위치값을 월드공간의 형태로 변환시킴
		// 해당 마우스는 스크린이라는 2D에 있는 존재이기 때문에 3D 공간상의 깊이값을 특정할 수 없음
		// 따라서 마우스의 위치를 near와 far에서 구하여 두 위치 사이를 잇는 방향벡터를 기반으로 레이를 만들어 체크
		...
		if (m_selected)
		{
			...
			m_cursorSphere.m_basicVertexConstantData.projection = projRow.Transpose();
			...
			if (m_dragStartFlag)
			{ // 드래그를 시작하는 경우
				// Update 함수이므로 프레임마다 해당 함수를 실행시킴
				// 따라서 left 버튼이 눌리자마자 실행되는 첫 프레임은 해당 부분을 실행함
			...
			{
				// 최초 호출 이후로는 마우스 버튼을 누르고 있다면
				// m_leftButton = true / m_dragStartFlag = false 상태
				Vector3 currentVector = pickPoint - m_mainBoundingSphere.Center; // TODO:
	...
	// 행렬에 별도로 저장되어 있는 이동값(Translation)을 추출함
