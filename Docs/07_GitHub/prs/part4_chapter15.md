...
- `Ex1501`, `Ex1502`와 10초 지연 `Ex1503` centered client-visible screenshot 3개를 `Docs/_assets/captures`로 승격
...
- SPH accumulation evidence: immediate frame보다 10초 지연 frame이 density/pressure/viscosity force 적용 후 particle cluster가 하단에 쌓이는 상태를 더 명확하게 보여 준다.
- Rendered-only asset boundary: `Ex1502`의 원본 `flare0.dds`는 직접 링크하지 않고, 직접 실행한 screenshot만 evidence로 사용한다.
...
Source에서 활성화한 particles에 buoyancy를 적용하고 `flare0.dds` sprite texture를 pixel shader에 바인딩해 fire-like particles를 표시한다. 원본 texture는 직접 공개하지 않고 rendered screenshot으로만 결과를 설명한다.
...
좌우 source에서 particles를 공급하고 `SphSimulation::Update`에서 density, pressure와 viscosity force를 계산한다. 10초 지연 capture를 selected evidence로 사용해 particle accumulation을 보여 준다.
...
`Ex1503_SphWater`는 좌우 source에서 공급된 particles에 SPH density/pressure/viscosity force와 boundary collision을 적용한다. 10초 지연 screenshot은 particles가 하단에 쌓이는 selected simulation 상태를 대표한다.
...
- Particle movement와 SPH stability는 video 후보지만 현재 PR은 static screenshot evidence 중심으로 정리한다.
