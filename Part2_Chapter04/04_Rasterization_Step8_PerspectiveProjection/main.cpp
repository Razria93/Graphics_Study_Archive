...
int main() {
    ...
    HWND hwnd = CreateWindow(wc.lpszClassName,
                             L"ComputerGraphics - Step8 PerspectiveProjection",
    ...
    while (WM_QUIT != msg.message) {
        if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
            ...
            ImGui::Checkbox(
                "usePerspectiveCorrectInterpolation",
                &example->rasterization.usePerspectiveCorrectInterpolation);
            ImGui::SliderFloat("distEyeToScreen",
                               &example->rasterization.distEyeToScreen, 0.5f,
                               5.0f);
            ImGui::SliderFloat("Left Square Z",
                               &example->rasterization.meshes[0]
                                    ->translation.z,
                               0.1f, 5.0f);
            ImGui::SliderFloat("Right Square Z",
                               &example->rasterization.meshes[1]
                                    ->translation.z,
                               0.1f, 5.0f);
            ImGui::End();
            ImGui::Render();
