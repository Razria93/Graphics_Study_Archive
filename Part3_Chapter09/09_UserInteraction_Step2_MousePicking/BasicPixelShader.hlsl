...
cbuffer BasicPixelConstantData : register(b0)
{
    ...
    bool isPickingSphere;
    // 단순히 ConstantsBuffer의 데이터만 추가하였는데 해당 증상이 다시 발발하고 있다
    // 문제 : 
    // bool이 HLSL에서는 4바이트 (무조건 4바이트 단위) / c++에서도 구조체 정렬규칙에 의해 패딩이 채워질 수 있음
    // 해결방안 :
    // 따라서 C++에서도 가능한 4바이트 단위로 구조체를 구성하며 패딩 또한 4바이트로 구성하도록 한다
};
...
PixelShaderOutput main(PixelShaderInput input)
{
    ...
    if (isPickingSphere)
    {
        diffuse += float4(1.0, 0.0, 0.0, 0.0);
    }
    ...
    // output.pixelColor = float4(input.indexColor, 1.0);
    output.indexColor = float4(input.indexColor, 1.0);
    // output.indexColor = float4(0.0, 1.0, 0.0, 1.0); //TODO:
    
    // 현재 피킹을 위한 staging의 Buffer에 색상을 넘기는 것은 제대로 구현되어 있음
    // 근데 문제는 렌더 타겟 두개 다 같은 이미지가 렌더링 되고 있다는 것
