# Status: 06 GraphicsPipeline Step8 ResizingWindow
...
- 주요 구현: `WM_SIZE`, swap chain `ResizeBuffers()`, render target/depth stencil view 재생성, viewport resizing
...
- Debug x64: 성공
- Release x64: 성공
- Release build note: raw project의 HLSL Release shader type 설정이 Step7과 같은 형태로 누락되어 있었고, archive `.vcxproj`에 Release `ShaderType`과 필요한 `ShaderModel` 설정을 추가했습니다.
...
- 다음 후보는 `06_GraphicsPipeline_Step9_PhongVsBlinnPhong`입니다.
