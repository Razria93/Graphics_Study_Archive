...
void CubeMapping::Initialize(ComPtr<ID3D11Device> &device,
    ...
    std::reverse(cubeMeshData.indices.begin(), cubeMeshData.indices.end()); // 안쪽에서 렌더링이 되어야하기 때문에 face를 바꿔준것

    // Object Sphere 혹은 Object Cube와 별도의 색상을 가져야함
    for (int i = 0; i < cubeMeshData.vertices.size(); i ++)
	{
		cubeMeshData.vertices[i].indexColor = Vector3(0.0f, 1.0f, 1.0f);
	}
    ...
    vector<D3D11_INPUT_ELEMENT_DESC> basicInputElements = {
        ...
        {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 4 * 3 + 4 * 3,
         D3D11_INPUT_PER_VERTEX_DATA, 0},
	    {"INDEXCOLOR", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 4 * 3 + 4 * 3 + 4 * 2,
	     D3D11_INPUT_PER_VERTEX_DATA, 0},
    };
...
void CubeMapping::Render(ComPtr<ID3D11DeviceContext> &context) {
    ...
    // 배경 자체를 렌더링할 때는 original 텍스춰 사용
