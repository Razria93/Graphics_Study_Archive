...
// HERE : EXR 전용 리드 이미지 함수
// EXR일경우 pixelformat도 float로 변경
void ReadEXRImage(const std::string filename, std::vector<uint8_t> &image,
    ...
    // 데이터 범위 확인해보는 방법
    // C++ 문법에는 f16이 없음
    // 그래서 f16을 디버깅하기 위해서는 f32로 변환해서 출력하거나 관찰해야함 (f16 라이브러리 활용)
    vector<float> f32(image.size() / 2);
    // (uint16_t *) 는 메모리를 의미함
    // 형태는 모르겠지만 그냥 데이터 처리를 16비트로 하곘다
    uint16_t *f16 = (uint16_t *)image.data();
    for (int i = 0; i < image.size() / 2; i++) {
        f32[i] = fp16_ieee_to_fp32_value(f16[i]); // f16_ieee (ieee는 표준이라는 뜻) 를 32비트 float로 바꿔주는 함수
    }

    // 최근에는 float가 16비트(f16)로도 충분히 활용가능하다 하여 16비트 float가 많이 사용되고 있음
    // Half-precision floating-point format
    // 주로 Half라고 많이 부름. 16비트짜리 float.
    ...
    // for (int i = 0; i < image.size() / 2; i++) {
    //     f16[i] = fp16_ieee_from_fp32_value(f32[i] * 2.0f); -> f32[i] * 2.0f 색상을 두배?
    // }
}
