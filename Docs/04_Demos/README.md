...
이 폴더는 demo capture/video evidence 계획과 시연 우선순위의 정본 위치다. 실제 build/run 검증 상태는 `Docs/03_Verification`을 기준으로 하고, 검토된 asset 파일은 `Docs/_assets`에 둔다.
...
- public 공개 가능성 판단은 `Docs/06_Publication`으로 분리한다.
