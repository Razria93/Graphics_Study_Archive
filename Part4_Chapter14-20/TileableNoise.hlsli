...
{
    // 연속적인 랜덤 필드
    //  - 부드럽게 랜덤 그래프를 여러개 겹쳐서 복잡하고 자연스러운 무늬를 만드는 함수
    // p        : 노이즈를 샘플링할 3D 좌표
    // freq     : 처음에 사용할 주파수 (무늬 크기)
    // octaves  : 몇 번 겹칠지 (층 개수)
    
    
    float G = exp2(-.85);   // 진폭을 줄이는 비율 (보통 0.5 ~ 0.7)
    float amp = 1.;         // 지금 추가할 노이즈의 세기
    float noise = 0.;       // 전체 결과 누적 값
    ...
    for (int i = 0; i < octaves; ++i)
    {
        ...
        // 1. 현재 주파수로 노이즈를 계산
        // 2. 해당 노이즈를 추적
        // 3. 다음에는 주파수를 2배로 (더 촘촘한 무늬)
        // 4. 다음에는 진폭을 줄임
        // 같은 노이즈 패턴(함수)을 주파수와 진폭만 다르게 재탕하여 여러번 겹쳐놓은 것
        //  - gradientNoise함수는 hash33이라는 함수가 가지고 있는 형태를 기반으로 주파수와 진폭을 바꿔가며 누적하는 함수
    }
