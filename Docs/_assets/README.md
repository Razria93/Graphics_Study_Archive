...
- capture/result 파일은 `Docs/03_Demos`에서 설명하고 이 폴더에서는 파일 위치와 용도만 관리한다.
- build/run 상태는 `Docs/02_Verification`으로 링크한다.
- public 후보 판단은 `Docs/05_Publication`으로 분리한다.
- `Docs/_assets`는 raw asset archive가 아니라 검증 완료 capture/result 보관 위치다.
...
| [videos](videos/README.md) | published video reference와 운영 안내 |
...
- 파일명과 문서 연결만으로 어떤 Part/Chapter/Portfolio capture/result인지 알 수 있다.
