...
void Ex1606_HybridWater::ParticleStep()
{
	...
	// Update particle velocity from grid velocity
	{
		...
		m_context->CSSetShader(m_diffUpSampleCS.Get(), 0, 0);	// 속도의 차이를 입자에 적용함 (해상도가 더 높기 때문)
	...
	// 새로운 입자를 만들고 속도를 지정해서 초기속도를 초기화함
	{
	...
	// Sorting & Update first indices 
	// Sort particles by grid index
	// 셀의 중심위치에 가까운 입자들만 찾아서 for문을 돌릴 때 효율적으로 하기 위함 (파티클에서 radius역할)
	...
	{
		ID3D11ShaderResourceView* srvs[3] = {m_particles.GetSRV(),		// 입자
		                                     m_sort.m_array.GetSRV(),	// 가속구조
		                                     m_firstIndex.GetSRV()};	// 가속구조
		ID3D11UnorderedAccessView* uavs[3] = {
		    m_velocity.GetUAV(), m_density.GetUAV(), m_signedDistance.GetUAV()};	// 입자의 정보를 격자(셀)들의 중심으로 샘플링함
...
void Ex1606_HybridWater::Render()
{
	...
	// Marching Cubes (삼각형 여러개로 그릴 때) [SDF -> TriangleMesh]
	if (m_renderMarchingCubes)
	{
		...
		m_context->IASetVertexBuffers(0, 1, m_dummyVertexBuffer.GetAddressOf(),
		                              &strides, &offsets);
		m_context->VSSetShader(m_mcVS.Get(), 0, 0);	// VS
		m_context->GSSetShader(m_mcGS.Get(), 0, 0);	// GS
		...
		m_context->OMSetRenderTargets(1, m_floatRTV.GetAddressOf(),
		                              m_defaultDSV.Get());
		m_context->Draw(UINT(m_widthUp * m_heightUp * m_depthUp), 0);	// 이름만 up이고 upSampling은 구현 안되어 있음
