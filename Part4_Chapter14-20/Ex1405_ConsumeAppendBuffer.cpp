...
void Ex1405_ConsumeAppendBuffer::Render()
{
	...
	// initCounts는 각 버퍼의 파티클이 몇개인지 설정하는 것
	UINT initCounts[2] = {UINT(m_consume.m_cpu.size()),	// m_consumeBuffer 
	                      0};							// m_appendBuffer
														// 결과적으로 모든 particle들을 consume으로부터 꺼내서 처리 후 append
														// 끝나면 consume이 비고 append가 꽉차게 됨
														// 이 때, consume과 append를 교체하고 다시 진행
	// 원래는 initCounts 부분이 NULL
	...
	m_context->Draw(UINT(appendCount), 0);	// <- AppendCount 만큼만 그리기 (C++ 아토믹 변수 및 연산)

	// 지금은 GPU에서 CopyStructureCount & Download로 GPU로부터 appendCount를 CPU로 받아와서 Draw로 다시 GPU로 보내는 형식
	// DrawIndexedInstancedIndirect()를 사용하면 AppendCount를 CPU로 복사하지 않아도 됩니다. [Indirect()]
	...
	// 프레임마다 두 버퍼가 Consume, Append 역할을 바꿔가며 주거니 받거니하면서 처리함
