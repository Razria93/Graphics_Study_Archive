...
float3 LightRadiance(Light light, float3 posWorld, float3 normalWorld, Texture2D shadowMap)
{
    ...
    if (light.type & LIGHT_SHADOW)
    {
        ...
        // light.viewProj 사용
        float4 lightProj = mul(float4(posWorld, 1.0), light.viewProj);
        float4 lightScreen = lightProj / lightProj.w;
        
        // 2. 카메라(광원)에서 볼 때의 텍스춰 좌표 계산
        // [-1, 1]x[-1, 1] -> [0, 1]x[0, 1]
        // 주의: 텍스춰 좌표와 NDC는 y가 반대
        //float2 lightTexcoord = 0.5 * (lightScreen.xy + float2(1.0, -1.0)) * float2(1.0, -1.0);
        ...
        // 3. 쉐도우맵에서 값 가져오기
        // shadowPointSampler 는 정해진 범위가 있음. 지정된 범위 이외에는 어두워지도록.
        float depth_Map = shadowMap.Sample(shadowPointSampler, lightTexcoord).r;
        float depth_ndc = lightScreen.z;
        
        // 4. 가려져 있다면 그림자로 표시
        // 힌트: 작은 bias (0.001 정도) 필요
        
        // Shadow acne
        // 원인 : lightMap의 pixel의 정수규격으로 인해 발생하는 현상
        // 이를 방지하기 위해 0.001 같은 eps를 두는 것
        
        // if (abs(depth_Map - depth_ndc) > 0.001)
        if(depth_Map + 0.001 < depth_ndc)
           shadowFactor = 0.0; // <- 0.0의 의미는?
    }

    // shadowFactor값이 낮아질수록 radiance가 줄어들어서 어두워짐
...
PixelShaderOutput main(PixelShaderInput input)
{
    ...
    // 임시로 unroll 사용
    [unroll] // warning X3550: sampler array index must be a literal expression, forcing loop to unroll (루프변수 사용 X)
             // 만약 여러개의 구조에 여러개의 쉐도우맵을 만들어 사용하고 싶을 경우 Texture2DArray 사용
    for (int i = 0; i < MAX_LIGHTS; ++i)
    {
        if (lights[i].type)
        {
            ...
            // DX11 해당 버전에서는  shadowMaps[i]의 인덱싱을 자유롭게 할 수 없음
            // 따라서 unroll을 통해 for문을 풀어서 사용
    ...
    // ambientLighting  : IBL (그림자 구현 X, 대신 AO로 어두운 부분을 구현)
    // directLighting   : point, spot, direction
    // emission         : 자체 발광
