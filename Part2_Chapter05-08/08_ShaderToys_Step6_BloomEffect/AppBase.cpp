...
{
	return float(std::max(1, m_screenWidth - m_guiWidth)) /
...
bool AppBase::InitMainWindow()
{
	...
	m_mainWindow = CreateWindow(wc.lpszClassName,
	                            L"ComputerGraphics - Chapter08 Step6 BloomEffect",
	                            WS_OVERLAPPEDWINDOW,
	                            100,                // 윈도우 좌측 상단의 x 좌표
	                            100,                // 윈도우 좌측 상단의 y 좌표
...
bool AppBase::CreateRenderTargetView()
{
	...
	// 여기서 Shader들이 접근해줘야하기 때문에 SRV를 만들어줌
	if (backBuffer)
	{
...
bool AppBase::ResizeClientResources(UINT width, UINT height)
{
	...
	if (!CreateRenderTargetView())
		return false;
	D3D11Utils::CreateDepthBuffer(m_device, width, height, numQualityLevels,
	                              m_depthStencilView);
	if (!m_depthStencilView)
		return false;
	SetViewport();
