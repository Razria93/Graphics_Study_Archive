...
void ExampleApp::Update(float dt) {
    ...
    if (m_leftButton) {
        ...
        // Matrix inverseProjView = m_camera.GetProjRow().Invert() * m_camera.GetViewRow().Invert(); // 같은거긴 함
        Matrix inverseProjView = (viewRow * projRow).Invert();

        // ViewFrustum 안에서 PickingRay의 방향 구하기
        // 이런 부분들은 확실한 지점을 지정해놓고 직접 찍어보는게 디버깅에 도움이 많이 됨
        // m_cursorNdcX = 0.0f, m_cursorNdcY = 0.0f 같은 것
        ...
        if (m_selected) {
            ...
            Vector3 hitPoint = cursorWorldNear + dir * dist;
            
            // 충돌 지점에 작은 구 그리기
            Matrix modelMat =
                Matrix::CreateTranslation(hitPoint); // TODO:

            Matrix invTransposeRow = modelMat;
            invTransposeRow.Translation(Vector3(0.0f)); // 이건 왜하는거지?
            invTransposeRow = invTransposeRow.Invert().Transpose();
