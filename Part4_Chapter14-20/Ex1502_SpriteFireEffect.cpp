...
bool Ex1502_SpriteFireEffect::Initialize()
{
	...
	D3D11Utils::CreateVertexShaderAndInputLayout(m_device, L"Ex1502_SpriteFireEffectVS.hlsl", inputElements,
...
void Ex1502_SpriteFireEffect::Update(float dt)
{
	...
	for (auto& p : m_particlesCPU)
	{
		if (p.life < 0.0f && newCount > 0)
		{
			...
			// p.position = Vector3(cos(theta), -sin(theta), 0.0) * randomLife(gen) * 0.2f;
			p.position = Vector3(0.0f, 0.0f, 0.0f);
			p.velocity = Vector3(cos(theta), -sin(theta), 0.0) * randomSpeed(gen) * 0.3f; // 입자들마다 다르게 생성 됨 (velocity가 서로 다름)
			p.life = randomLife(gen) * 0.8f;                                              // life 추가
			// p.radius = 1.0f;
			p.radius = 0.5f + randomLife(gen) * 0.2f;
	...
	for (auto& p : m_particlesCPU)
	{
		if (AppBase::m_leftButton && p.life < 0.0f && newCount > 0)
		{
			...
			// p.position += Vector3(cos(theta), -sin(theta), 0.0) * randomLife(gen) * 0.05f;
			p.velocity = Vector3(cos(theta), -sin(theta), 0.0) * randomSpeed(gen) * 0.1f;
			p.life = randomLife(gen) * 0.8f; 
			// p.radius = 0.2f;
			p.radius = 0.1f + randomLife(gen) * 0.2f;

			newCount--;
		}
	}

	// const Vector3 gravity = Vector3(0.0f, -9.8f, 0.0f);	// 중력 : 아래로 내려가는 속도
	const Vector3 buoyancy = Vector3(0.0f, 1.0f, 0.0f); // 부력 : 위로 올라가는 속도
	...
	for (auto& p : m_particlesCPU)
	{
		...
		p.velocity = p.velocity + buoyancy * dt; // 중력가속도를 사용하여 속도를 업데이트 하고 있음
