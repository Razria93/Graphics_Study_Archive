...
void Graphics::InitBlendStates(ComPtr<ID3D11Device>& device)
{
	...
	// AlphaToCoverageEnable : true일 경우, 최종 alpha값에 따라 연산된 색상이 샘플에 얼마나 저장될지 결정됨
	// 여기서는 사실상 Alpha가 1.0이기 때문에 모든 sample을 갱신, 따라서 AlphaToCoverageEnable = true 의 의미가 없는 수준

	D3D11_BLEND_DESC mirrorBlendDesc;
	ZeroMemory(&mirrorBlendDesc, sizeof(mirrorBlendDesc));
	mirrorBlendDesc.AlphaToCoverageEnable = true;                             // 알파값(α)을 MSAA 샘플 커버리지(mask)로 변환
	mirrorBlendDesc.IndependentBlendEnable = false;                           // 개별 RenderTarget에 대해서 설정 (Multiple Render Target / 최대 8개)
	mirrorBlendDesc.RenderTarget[0].BlendEnable = true;                       // RTV의 Blend 가능 여부
	mirrorBlendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_BLEND_FACTOR;      // 소스 색상(SrcColor)에 곱해질 가중치  (BLEND_FACTOR : 외부 상수, OMSetBlendState() 두 번째 인자)
	mirrorBlendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_INV_BLEND_FACTOR; // 대상 색상(DestColor)에 곱해질 가중치 (INV_BLEND_FACTOR : 1 - 외부 상수, 위의 반대)
	mirrorBlendDesc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD;             // 색상 간 결합 연산 (ADD : 더하기)
	mirrorBlendDesc.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_ONE;          // 소스 알파에 곱해질 연산용 가중치  (srcAlpha * 1.0)
	mirrorBlendDesc.RenderTarget[0].DestBlendAlpha = D3D11_BLEND_ONE;         // 대상 알파에 곱해질 연산용 가중치  (destAlpha * 1.0)
	mirrorBlendDesc.RenderTarget[0].BlendOpAlpha = D3D11_BLEND_OP_ADD;        // 알파 간 결합 연산 (ADD : 더하기) (clamp가 있어서 0.0 ~ 1.0)

	// 필요하면 RGBA 각각에 대해서도 조절 가능
	mirrorBlendDesc.RenderTarget[0].RenderTargetWriteMask = // 출력할 색상 채널 제어
	...
	blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_BLEND_FACTOR;  // 외부 상수
	blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_BLEND_FACTOR; // 외부 상수 (INV 아님)
...
void Graphics::InitShaders(ComPtr<ID3D11Device>& device)
{
	...
	// {SemanticName,			: 시멘틱 이름 [VS에서 어떤 시멘틱으로 받을지 결정]
	//  SemanticIndex,			: 시멘틱 번호 [같은 시멘틱 여러 개 구분]
	//  Format,					: 데이터 형식 [데이터의 크기/자료형] 
	//  InputSlot,				: 입력 슬롯	 [어떤 버텍스 버퍼에서 읽을지]
	//  AlignedByteOffset,		: 시작 위치	 [버퍼 내부에서 시작 위치]
	//  InputSlotClass,			: 슬롯 클래스 [Vertex/Instance 데이터 구분, 갱신단위]
	//  InstanceDataStepRate}	: 인스턴스 데이터 변경 주기
	...
	vector<D3D11_INPUT_ELEMENT_DESC> grassIEs = {
	    // D3D11_INPUT_PER_VERTEX_DATA : 정점 1개마다 갱신하는 데이터
	    ...
	    // 인스턴스에 대해서 변환행렬을 보냄
	    // D3D11_INPUT_PER_INSTANCE_DATA : 인스턴스 1개마다 갱신하는 데이터
	    // 갱신 타이밍이 다르기에 버퍼슬롯을 0(Per Vertex), 1(Per Instance)로 분리함
	    ...
	    {"COLOR", 0, DXGI_FORMAT_R32_FLOAT, 1, 64,
	     D3D11_INPUT_PER_INSTANCE_DATA, 1}}; // 바람의 세기
	...
	D3D11Utils::CreateVertexShaderAndInputLayout(device, L"BasicVS.hlsl",
	                                             skinnedIEs, skinnedVS, skinnedIL,
	                                             vector<D3D_SHADER_MACRO>{{"SKINNED", "1"}, {NULL, NULL}}); // Create skinned VertexShader / InputLayout
	                                                                                                        // BasicVS와 거의 비슷하나 매크로를 지정할 수 있도록 되어 있음
	                                                                                                        // {NULL, NULL} : 문자열에서 \0 넣는 것과 같음. 매크로의 끝을 알림 (반드시 넣어야함)
	                                                                                                        // common에도 컴파일타임에 매크로가 추가됨
	...
	D3D11Utils::CreateVertexShaderAndInputLayout(device, L"DepthOnlyVS.hlsl",
	                                             skinnedIEs, depthOnlySkinnedVS, skinnedIL,
