...
// 가시화를 위해 속도 여러가지로 조절
static float dt = 0.005f; // ConstBuffer로 받아올 수도 있음
...
float2 getCurl(float2 uv, float2 dx)
{
    // TODO: 완성
    float2 dp;
    dp.x = (getNoise(uv + float2(dx.x, 0.0)) - getNoise(uv - float2(dx.x, 0.0))) / (2.0 * dx.x);
    dp.y = (getNoise(uv + float2(0.0, dx.y)) - getNoise(uv - float2(0.0, dx.y))) / (2.0 * dx.y);
