...
- 새로 작성하거나 수정하는 GitHub public body의 단계·숫자·날짜·Example 범위는 `부터 ... 까지` 문장으로 표기한다. `~`와 en dash(`–`)를 범위 표기로 사용하지 않는다. `~`는 code, 정규식, URL, path처럼 문법상 필요한 경우에만 사용한다. GitHub Markdown은 single tilde range를 의도하지 않은 취소선으로 해석할 수 있다. 기존 body의 전수 정리는 별도 정리 작업으로 분리한다.
