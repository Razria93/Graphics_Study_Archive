...
| `02_Bloom` Demo publication bundle | `Docs/03_Demos/Part1_Chapter01-02/02_Bloom.md`, `Docs/07_GitHub/issues/demo/demo_part1_chapter01-02.md`, `Docs/_assets/captures` | input/result와 구현 선택을 독립적인 시각·기술 설명으로 제공 가능 | 검토 필요 | 워터마크·개인 식별자·application UI 없음. Input APP1 metadata 제거 후 해상도와 decoded pixel 유지. Result PNG text/EXIF/XMP 없음. 이미지·코드 증거는 commit permalink로 고정 | 생성 날짜, prompt 원문, provenance screenshot, 작업용 원본 확인 | Generated input provenance 일부 미확인. Demo Issue 게시와 public 공개 확정은 별도 승인 대상 |
