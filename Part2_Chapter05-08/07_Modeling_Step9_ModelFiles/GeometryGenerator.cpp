                                                 ...
                                                 std::string filename)
{
	...
	float dx = vmax.x - vmin.x; // 박스 폭
	float dy = vmax.y - vmin.y; // 박스 높이
	float dz = vmax.z - vmin.z; // 박스 깊이

	float dl = XMMax(XMMax(dx, dy), dz);

	float cx = (vmax.x + vmin.x) * 0.5f; // 박스 현재 폭 중심 위치
	float cy = (vmax.y + vmin.y) * 0.5f; // 박스 현재 높이 중심 위치
	float cz = (vmax.z + vmin.z) * 0.5f; // 박스 현지 깊이 중심 위치

	for (auto &mesh : meshes)
	{
		for (auto &v : mesh.vertices)
		{
			// 원점 기준으로 vertex들을 정렬시킨 뒤 정규화 (박스의 가장 긴 축값만 1.0f, 나머지는 해당 축값을 기준으로 상대적인 비율이 됨)
