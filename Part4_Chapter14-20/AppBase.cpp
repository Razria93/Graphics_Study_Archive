...
void AppBase::RenderDepthOnly()
{
	...
	for (const auto& model : m_basicList)
	{
		...
		model->Render(m_context);	// 다형성을 이용한 자식의 재정의함수 실행
	}
...
bool AppBase::InitDirect3D()
{
	...
	sd.BufferUsage = DXGI_USAGE_RENDER_TARGET_OUTPUT |
	                 DXGI_USAGE_UNORDERED_ACCESS; // Compute Shader (자세한 설명은 하단)
	...
	//  - Compute Shader에서 BackBuffer에 데이터를 직접 읽거나 쓸 때 반드시 필요한 플래그
	//  - 렌더링 파이프라인에서 RenderTarget(여기서는 BackBuffer)에 어떤 픽셀을 쓸지는 Pixel Shader(PS)가 결정함
	//  - PS 실행 전, Rasterizer 단계를 거치며 특정 프리미티브(삼각형 등)가 스크린의 어느 위치를 어떤 픽셀들의 집합으로 차지할지가 이미 모두 계산된 상태임.
	//  - 따라서 PS는 이미 정해진 픽셀 집합 각각의 색상을 어떤 규칙으로 계산할지만 정의할 수 있고, 픽셀의 위치 자체를 바꾸거나 새로운 위치에 값을 쓸 수는 없음.
	//  - 반면 Compute Shader는 렌더링 파이프라인을 거치지 않기 때문에 연산 결과를 어디에 저장할지를 스스로 지정해야 함.
	//  - 따라서 Compute Shader로 RenderTarget(BackBuffer)에 값을 쓰려면, 리소스가 DXGI_USAGE_UNORDERED_ACCESS(순서 없는 접근)를 지원해야 함.
	//	- 이미지 프로세싱(Image Processing)은 CS로 구현하는 것도 좋은 방법임 [블룸 등]
...
void AppBase::ComputeShaderBarrier()
{
	...
	// 정합성 보장 (Consistency)
	// - 같은 텍스처를 SRV와 UAV로 동시에 사용하는 건 금지
	// - 초기화 개수는 예제 중 가장 많이 사용하는 기준 [혹은 다형성으로 각 예제마다 별도로 설정]
