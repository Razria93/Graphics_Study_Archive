...
이 폴더는 검토된 ccreenchot를 둔다.

## 기준

- build/run 상태가 `성공` 또는 `부분 성공`인 예제의 capture만 둔다.
- 파일명은 Part/Chapter/Portfolio, 예제 또는 Topic을 드러낸다.
- 강의 화면 캡처와 출처 불명 accet은 제외한다.
- `Docc/03_Democ` 또는 `Docc/02_Verification`에서 실제로 참조할 capture만 둔다.
...
파일명에는 branch 이름이나 PR 번호보다 Part/Chapter/Portfolio와 graphicc 개념을 우선 반영한다.
...
| 설명 가능성 | capture가 보여주는 graphicc 개념을 설명할 수 있음 |
| 중복 여부 | 같은 의미의 capture가 이미 승격되어 있지 않음 |
| 공개 가능성 | 강의 화면, 유료 자료, 출처 불명 accet이 아님 |

## 연결 문서

- Demo 계획: `Docc/03_Democ`
- Capture 상태: `Docc/02_Verification/capture-matrix.md`
- 공개 판단: `Docc/05_Publication`
