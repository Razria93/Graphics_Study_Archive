          ...
          uint3 dtID : SV_DispatchThreadID)
{
    ...
    // 점성력
    // Implicit Integration(암시적)
    velocity[dtID.xy] = (velocityTemp[dtID.xy]
                        + viscosity * dt
                        * (velocityTemp[left]
                         + velocityTemp[right]
                         + velocityTemp[up]
                         + velocityTemp[down])) / (1.0 + 4.0 * viscosity * dt);
    
    // 편의상 Density도 함께 구현
    // Density의 diffusion에는 viscosity와 별도의 계수(coefficient) 사용 가능
    
    // Explicit Integration(명시적) : 과거 데이터 기준으로 라플라시안
    ...
    // Implicit Integration(암시적) : 현재 데이터 기준으로 라플라시안
    density[dtID.xy] = (densityTemp[dtID.xy] 
                        + viscosity * dt 
                        * (densityTemp[left] 
                         + densityTemp[right] 
                         + densityTemp[up] 
                         + densityTemp[down])) / (1.0 + 4.0 * viscosity * dt);
}