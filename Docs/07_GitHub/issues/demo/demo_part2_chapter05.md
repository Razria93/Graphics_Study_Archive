...
`Part2_Chapter05-08` Chapter05는 GLM 기반 matrix와 affine transformation을 다룬다.

학습 흐름은 이를 DirectXMath와 SimpleMath 환경으로 옮기는 과정이다.

서로 다른 vector convention에 맞춰 matrix composition 순서를 구성한다.

Step2와 Step4는 같은 transform 의도와 lighting 결과를 만들 수 있음을 확인한다.
...
Inverse-transpose normal matrix를 분리한다.

변형된 geometry의 면별 Directional Light 결과를 유지한다.
...
API와 multiplication 표기는 달라진다.

대응하는 silhouette와 lighting 의도는 유지한다.
...
Non-uniform scale에서는 model matrix 대신 inverse transpose를 normal에 적용한다.

이 처리는 surface와 normal의 직교 관계를 보존한다.
