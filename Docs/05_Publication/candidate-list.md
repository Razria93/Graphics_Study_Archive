...
| Build/run 검증 요약 | `Docs/02_Verification` | 직접 확인한 결과를 포트폴리오 확인 자료로 사용 가능 | 검토 필요 | `미확인` 항목은 공개 설명에 사용하지 않음 |
| Demo capture/result | `Docs/03_Demos`, `Docs/_assets` | 직접 실행 결과 capture/result로 사용 가능 | 검토 필요 | `02_Bloom` input/result 비교 이미지를 `Docs/_assets/captures`에 승격함. public 공개 확정은 아님 |
