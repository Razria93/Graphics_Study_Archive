...
bool ExampleApp::Initialize()
{
	...
	// 환경 박스 초기화
	// 다른 물체들과 달리 시점이 물체 안에 있기 때문에 삼각형의 Winding규칙을 바꿔 전면을 안쪽으로 바꿔줘야함
	{
	...
	// 바닥
	{
		...
		// m_basicList :
		// 거울과 같이 별도로 특수하게 렌더링을 해야하는 경우를 제외하고는
		// 모두 basicList에 등록해서 렌더링
	...
	{
		// 표시를 하는 obj는 조명의 영향을 받으면 안되기 때문에 색을 emission으로 설정해주고 있음
		MeshData sphere = GeometryGenerator::MakeSphere(0.01f, 10, 10);
		m_cursorSphere =
...
void ExampleApp::Update(float dt)
{
	...
	{
		// MainObject에 보이지 않는 BoundingSphere를 기반으로 Obj를 조작하는 기능
		// 이후에는 저마다의 물체마다 BoundingBox, Sphere를 가지고 있게 구현할 것임
		Quaternion q;
...
void ExampleApp::Render()
{
	...
	// 환경에 사용하는 맵은 모두가 쓰이는 것
	// 공용 텍스춰들: "Common.hlsli"에서 register(t10)부터 시작
	vector<ID3D11ShaderResourceView*> commonSRVs = {m_envSRV.Get(),
	                                                m_specularSRV.Get(),
	                                                m_irradianceSRV.Get(),
	                                                m_brdfSRV.Get()};

	// 공용으로 사용하는 쉐이더들은 t10부터 시작하는 것으로 설정해둠
	...
	// RTV의 경우 시점만 바꿔서 해당 RV에 다시 그리는 경우도 있음
	// 따라서 PSO에 포함되지 않고 따로 분리가 되어 있음
	// ex. 거울 / 미니맵 / 드론 / 후방카메라 등
	...
	// 기본 1-1. 거울은 빼고 원래 대로 그리기
	// SetPipelineState 함수는 PSO에 지정되어 있는 내부 요소를 꺼내서 각 쉐이더나 state에 설정해주는 함수
	// 매번 길게 따로 설정하던 것을 PSO와 함수 하나로 깔끔하게 정리됨
	AppBase::SetPipelineState(m_drawAsWire ? Graphics::defaultWirePSO
	                                       : Graphics::defaultSolidPSO);

	// GlobalConst를 선택하고 함수 내부에서 해당 설정값을 풀어서 적용시키는 것
	// DefaultWorld와 MirriorWorld가 같은 자원으로 Const만 달리해서 그렸던 것을 생각하면 됨
	// 기존에는 Render함수에서 VSSetConstBuffer를 했으나 이제는 별도의 함수로 통합적으로 실행함
	AppBase::SetGlobalConsts(m_globalConstsGPU);

	for (auto& i : m_basicList)
	{
		// 객체에 설정된 값을 기반으로 렌더를 함
		i->Render(m_context);
	}

	// 기본 1-2. 노멀 그리기
	// 이렇게 PSO별로 렌더링을 하게 될 경우 개발자 입장에서는 일관성이 있어서 좋고,
	// 실 성능면에서도 같은 작업을 쉐이더 변경없이 일괄적으로 처리하는 것이기 때문에 효율적임
	...
	// 기본 1-3. 스카이박스 그리기
	// PSO를 만들어둚으로서 코드 전체에서 CubeMapping 이라는 클래스를 없애고 해당 내용을 통합함
	AppBase::SetPipelineState(m_drawAsWire ? Graphics::skyboxWirePSO
	                                       : Graphics::skyboxSolidPSO);

	// 다른 물체들과 똑같이 다른 모델과 같은 알고리즘을 따름
	m_skybox->Render(m_context);

	// TODO :
	// 거울 1. 거울 위치만 StencilBuffer에 1로 표기
	m_context->ClearDepthStencilView(m_depthStencilView.Get(),
	                                 D3D11_CLEAR_STENCIL,
	                                 1.0f, 0);
