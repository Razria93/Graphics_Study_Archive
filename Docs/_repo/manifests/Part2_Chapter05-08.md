...
- Build/run status: Step1 Matrix(GLM), Step2 Lights(GLM), Step3 DirectXMath, Step4 Lights(SimpleMath), Pipeline Step1 COM 성공 / Pipeline Step2 InitializingD3D 빌드 성공, 실행 미확인
...
| `06_GraphicsPipeline_Step2_InitializingD3D` | 반영 완료 | 빌드 성공 / 실행 미확인 | 검토 필요 | D3D11 initialization, swap chain, colored cube |
| `06_GraphicsPipeline_*` | 예정 | 미확인 | 검토 필요 | Step3 이후 D3D11 pipeline |
...
1. 사용자가 `06_GraphicsPipeline_Step2_InitializingD3D` Debug/Release 실행 확인을 완료하면 status 문서와 Part2 README를 갱신합니다.
2. 다음 후보 `06_GraphicsPipeline_Step3_ConstantBuffer`의 raw 구조와 dependency를 확인합니다.
