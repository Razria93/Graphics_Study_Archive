...
GLM은 column-vector convention에 맞춰 오른쪽 matrix부터 적용하고, SimpleMath는 row-vector convention에 맞춰 반대 방향으로 composition을 구성한다. 이 선택은 같은 transform 적용 순서를 서로 다른 API 표현으로 연결한다.
...
Position은 translation의 영향을 받는 homogeneous coordinate로, normal은 방향 vector로 변환한다. Non-uniform scale에서는 model matrix 대신 inverse transpose를 normal에 적용해 surface와의 직교 관계를 보존한다.
