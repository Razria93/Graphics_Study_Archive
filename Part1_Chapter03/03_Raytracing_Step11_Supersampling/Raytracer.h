...
namespace hlab
{
    ...
    public:
        ...
        {
            auto sphere = std::make_shared<Sphere>(vec3(1.0f, 0.0f, 1.5f), 0.8f);
        ...
        vec3 SampleAmbient(const Hit &hit) const
        {
            ...
            {
                return hit.obj->amb * hit.obj->ambTexture->SamplePoint(hit.uv);
            }
        ...
        vec3 traceRay(Ray &ray)
        {
            ...
            vec3 pointColor = SampleAmbient(hit);
        ...
        vec3 TraceRay2x2(const vec3 &eyePos, const vec3 &pixelCenter, const float pixelSize, const int depth)
        {
            if (depth == 0)
            {
                Ray ray{pixelCenter, glm::normalize(pixelCenter - eyePos)};
                return traceRay(ray);
            }

            const float childSize = pixelSize * 0.5f;
            const vec3 topLeft = pixelCenter - vec3(childSize, childSize, 0.0f);
            vec3 color(0.0f);
            ...
            for (int y = 0; y < 2; ++y)
            {
                for (int x = 0; x < 2; ++x)
                {
                    const vec3 childCenter = topLeft + vec3((x + 0.5f) * childSize, (y + 0.5f) * childSize, 0.0f);
                    color += TraceRay2x2(eyePos, childCenter, childSize, depth - 1);
                }
            }
        ...
        void Render(std::vector<glm::vec4> &pixels)
        {
            ...
            const float pixelSize = 2.0f / this->height;
            const int supersamplingDepth = 3;
