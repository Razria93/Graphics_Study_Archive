...
| VI-005 | Part4_Chapter14-20 | 조건부 해소 | Debug x64 build 후 ignored output의 runtime DLL이 정리된다. 2026-08-07에는 vcpkg `x64-windows/debug/bin` DLL 23개를 `x64/Debug`에 복구한 뒤 Chapter16부터 Chapter20까지 smoke를 수행함 | build output DLL copy 방식이 생기기 전까지 실행 전 runtime DLL 복구 절차를 Part4 build/run 기준으로 유지 |
