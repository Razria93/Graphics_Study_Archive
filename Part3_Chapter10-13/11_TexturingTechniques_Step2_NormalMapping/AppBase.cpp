...
void AppBase::UpdateMousePickColor() { // Picking
    ...
    // 텍스춰 메모리에서 읽어올 때 범위 오류 방지
    m_cursorX = std::clamp(m_cursorX, 0, m_screenWidth - 1);
    m_cursorY = std::clamp(m_cursorY, 0, m_screenHeight - 1);
...
LRESULT AppBase::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_SIZE:
        ...
        if (m_swapChain && LOWORD(lParam) > 0 && HIWORD(lParam) > 0) { // 처음 실행이 아닌지 확인
...
bool AppBase::InitMainWindow() {
    ...
    m_mainWindow = CreateWindow(wc.lpszClassName, L"ComputerGraphics - Chapter11 Step2 NormalMapping",
                                WS_OVERLAPPEDWINDOW,
                                100, // 윈도우 좌측 상단의 x 좌표
                                100, // 윈도우 좌측 상단의 y 좌표
