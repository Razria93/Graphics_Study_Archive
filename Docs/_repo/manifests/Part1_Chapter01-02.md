...
`01_DX11InitAndImGui`는 manifest 기준으로 선별 복사했습니다. `02_Bloom`은 입력 이미지 출처/라이선스 확인 전까지 아직 복사하지 않습니다.
...
| 경로 의존성 | `01_DX11InitAndImGui` project item은 상대 경로 기준으로 확인됨. 실제 build는 미확인 | 부분 확인 |

## Next action

1. `01_DX11InitAndImGui`를 Visual Studio에서 `Debug x64`로 빌드할 수 있는지 확인합니다.
2. 빌드/실행 결과를 [build-guide.md](../tracking/build-guide.md)에 기록합니다.
3. `02_Bloom`은 입력 이미지 출처/라이선스 확인 전까지 asset을 `검토 필요`로 둡니다.
