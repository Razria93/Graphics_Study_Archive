# Import Integrity Checklist

이 문서는 raw 예제를 archive로 옮긴 뒤, 이번 `07_Modeling_Step4_Cylinder`에서 발생한 문제를 반복하지 않기 위한 마무리 체크리스트입니다.
...
| HLSL 첫 문자 | shader 파일 첫 non-empty 문자가 `#`, `struct`, `cbuffer`, `Texture2D` 등 정상 코드인지 확인 | 파일 앞쪽의 깨진 문자, 잘못 들어간 빈 binary/text 변환 흔적 제거 |
...
- 실행 편의를 위해 `Image` item을 `Content + CopyToOutputDirectory`로 바꿨지만, 사용자의 요청에 따라 raw project 구조를 우선하도록 되돌렸습니다.
