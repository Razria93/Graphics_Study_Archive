...
MeshData GeometryGenerator::SubdivideToSphere(const float radius, MeshData meshData)
{
	...
	{
		// '원점을 중심으로 한 구'는 normal에 radius를 곱하면 반지름이 radius인 구의 표면의 위치가 됨
		// 우리가 결과값으로 반환해야할 Sphere는 radius인 Sphere 이므로 이에 맞게 position값을 재조정하는 것임
		v.position = v.normal * radius;
	}

	// 구의 표면으로 옮기고 노멀 계산하는 람다함수
	// ProjectVertex가 함수 이름, (Vertex &v)는 매개변수 형태, [&]는 매개변수로 들어온 데이터를 어떻게 사용할 것인지
	auto ProjectVertex = [&](Vertex &v)
	{
		// 원점에서 v.position을 향하는 벡터를 노멀라이즈 해서 노멀 벡터를 만들어줌
		// 이를 기반으로 radius를 곱하여 다시 구 표면의 점으로 재조정함
		// 이를 하는 이유는 삼각형을 쪼기개만 하면 삼각형 위의 점만 늘어난 것처럼 됨
		// 그리고 그 점들은 원점으로부터의 거리가 radius보다 작을 것임
		// 해당 거리를 radius로 보정하여 제 위치를 찾게 하는 것
		v.normal = v.position;
		v.normal.Normalize();
		v.position = v.normal * radius;
	...
	for (size_t i = 0; i < meshData.indices.size(); i += 3)
	{
		...
		{
			// 버텍스를 그릴 순서에 맞게 생성하기 때문에 인덱스는 0부터 11까지 순서대로 입력
			// 2번째부터는 count값이 offset이 되어 계속 추가됨
			newMesh.indices.push_back(j + count);
		}
		count += 12;
	}

	// 결과적으로 데이터로 받은 vertex, index 데이터를 기반으로 vertex_new, index_new를 
	// 새로 구성하여 매개변수로 넘어왔던 MeshData 변수를 덮어씌우는 방법
	// 도형이 하이폴로 그린것 보다 찌그러지는 이유는 삼각형의 중심값이 구 표면에서의 중심값은 아니기 때문인거 같음
	return newMesh;
}
} // namespace hlab