...
Chapter20은 character fire animation notify frame에서 PhysX projectile을 만든다.

같은 notify 흐름에서 fire billboard도 생성한다.

Physics simulation pose는 render transform으로 동기화한다.

대표 visual은 `Ex2001_GamePlay` 시연 video의 timestamp frame storyboard로 구성한다.

Character, animation, texture 원본 asset은 직접 게시하지 않는다.
...
`Ex2001_GamePlay`은 fire animation notify frame에서 projectile actor를 생성한다.

같은 frame에서 fire billboard도 만든다.

Dynamic object pose는 render world matrix에 반영한다.

Storyboard는 4.260s, 14.200s, 24.140s frame을 순서대로 기록한다.
...
Fire input edge가 cooldown 조건을 만족하면 animation state를 초기화한다.

Frame index도 함께 초기화한다.

Animation frame이 notify frame에 도달하면 `FireProjectile`을 호출한다.

이 함수는 render position을 physics position으로 변환한다.

Dynamic sphere actor와 fire billboard model도 생성한다.
...
Character animation을 갱신한 뒤 PhysX scene을 fixed time step으로 simulate한다.

그 result를 fetch한다.

Projectile을 포함한 dynamic actor pose는 render scale로 변환된다.

변환 결과는 world matrix와 constant buffer에 반영된다.
...
- Projectile effect는 billboard와 PhysX sphere actor의 pair다.
- Impact event, damage system과 collision callback은 구현하지 않는다.
