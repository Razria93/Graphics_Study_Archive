...
이 문서는 build/run 검증과 연결되는 capture 상태를 요약한다. 실제 demo 계획과 evidence는 `Docs/03_Demos`를 정본으로 둔다.
...
- capture는 build/run 성공 또는 부분 성공 상태와 연결한다.
- 강의 화면 캡처와 출처 불명 asset은 evidence 후보로 두지 않는다.
