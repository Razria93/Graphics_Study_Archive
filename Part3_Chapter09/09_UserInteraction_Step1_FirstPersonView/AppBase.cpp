...
void AppBase::OnMouseMove(WPARAM btnState, int mouseX, int mouseY)
{
	...
	// 마우스는 Screen좌표계에 있기 때문에 정수 좌표 기준 [0, width - 1], [0 ~ height - 1] 의 범위 내에 있음
	// 실수 좌표계 기준으로는 [0, width] [0, height] 지만 픽셀기준으로는 정수값 pair의 좌표계 기준으로 데이터가 저장되기 때문
	// 다만 NDC로 전환할 때는 실수좌표계 기준이니 실수좌표계의 스펙트럼인 [0 ~ width], [0 ~ height] 로 계산하는 것
...
LRESULT AppBase::MsgProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
		...
		{ // 처음 실행이 아닌지 확인
			...
			// LOWORD, HIWORD 모두 unsigned int 형
	...
	case WM_KEYDOWN:
		...
		// TODO: 이런 방식으로 구현하면 움직임이 부드럽지 않습니다.
		//  else if (wParam == 87)
