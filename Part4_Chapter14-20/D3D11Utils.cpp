...
void D3D11Utils::CreateVertexShaderAndInputLayout(
    ...
    const vector<D3D_SHADER_MACRO> shaderMacros) // Macro 매개변수
{
	...
	HRESULT hr = D3DCompileFromFile(
	    filename.c_str(), shaderMacros.empty() ? NULL : shaderMacros.data(),	// 쉐이더 코드(hlsl)에 매크로를 추가한 것처럼 컴파일 하라는 뜻 (ex. #define SKINNED 1 )
                                 ...
                                 ComPtr<ID3D11UnorderedAccessView>& uav)
{
	...
	D3D11_TEXTURE3D_DESC txtDesc; // 3D
	ZeroMemory(&txtDesc, sizeof(txtDesc));
	txtDesc.Width = width;
	txtDesc.Height = height;
	txtDesc.Depth = depth;	// 필수
	txtDesc.MipLevels = 1;
	txtDesc.Format = pixelFormat;
	txtDesc.Usage = D3D11_USAGE_DEFAULT;
	txtDesc.BindFlags = D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_RENDER_TARGET |
	...
	if (initData.size() > 0)
	{
		...
		ThrowIfFailed(device->CreateTexture3D(&txtDesc, &bufferData,
		                                      texture.GetAddressOf()));	// Texture3D
	}
	else
	{
		ThrowIfFailed(
    ...
    ComPtr<ID3D11UnorderedAccessView>& uav)
{
	...
	bufferDesc.StructureByteStride = sizeElement;                 // 구조체 바이트 간격	(간격의 크기는 구조체의 크기와 같음)
	                                                              // D3D11_RESOURCE_MISC_BUFFER_STRUCTURED 플래그를 설정하지 않으면 StructureByteStride는 의미없음
	bufferDesc.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_STRUCTURED; // Structured Buffer임을 명시하는 플래그 (StructureByteStride를 요소 크기로 해석하도록 설정)

	// 참고: Structured는 D3D11_BIND_VERTEX_BUFFER로 사용 불가

	if (initData)
	{
		// D3D11_SUBRESOURCE_DATA 에 대해서 질문
		// 여기서는 데이터가 복사가 안되는건지
		D3D11_SUBRESOURCE_DATA bufferData;
		ZeroMemory(&bufferData, sizeof(bufferData));
		bufferData.pSysMem = initData;
                                          ...
                                          ComPtr<ID3D11Buffer>& buffer)
{
	...
	desc.StructureByteStride = sizeElement;
	desc.MiscFlags = D3D11_RESOURCE_MISC_DRAWINDIRECT_ARGS; // <- IndirectArgs 전용 MiscFlags
                                    ...
                                    ComPtr<ID3D11UnorderedAccessView>& uav)
{
	...
	uavDesc.Buffer.Flags =
	    D3D11_BUFFER_UAV_FLAG_APPEND; // <- AppendBuffer로 사용 (이부분만 다름)
	device->CreateUnorderedAccessView(buffer.Get(), &uavDesc,
	                                  uav.GetAddressOf());
