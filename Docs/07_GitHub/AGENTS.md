...
- Chapter PR의 `핵심 개념`과 `대표 예제`는 표보다 문단과 짧은 리스트를 기본으로 한다.
- closeout, 구조 정리, 운영 규칙 변경 PR은 `주요 변경`을 사용하는 maintenance 형식으로 작성할 수 있다.
- maintenance PR에는 `핵심 개념`, `대표 예제`, `스크린샷`을 강제하지 않는다.
