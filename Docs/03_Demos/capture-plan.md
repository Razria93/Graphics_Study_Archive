...
- Step4 DrawingSphere와 Step5 PhongShading의 기본·파라미터 조정 screenshot, Step6 PerspectiveView부터 Step10 Texturing까지의 기본 screenshot을 동일한 전체 application window 규격으로 확보했다.
- Step10 Texturing은 저장소 도구로 만든 UV 진단 texture를 적용해 방향·bilinear sampling·Square seam 부재를 확인했다.
- Step11~14는 build/run과 문서화를 같은 순서로 완료한 뒤 capture 필요성을 판단한다.
