...
bool ExampleApp::InitializeCubeMapping()
{
	...
	// .dds 파일 읽어들여서 초기화
	ComPtr<ID3D11Texture2D> texture; // 임시로 사용함. 어짜피 리소스에 접근할 때 사용하는 것은 ResourceView
	auto hr = CreateDDSTextureFromFileEx(
	    ...
	    DDS_LOADER_FLAGS(false), (ID3D11Resource **)texture.GetAddressOf(), // 1. COM객체는 ID3D11Resource* 변수를 가지고 있음.
	    this->m_cubeMapping.cubemapResourceView.GetAddressOf(), nullptr);   // 2. GetAddressOf()는 그 변수를 가리키는 포인터인 ID3D11Resource**를 반환함.
	                                                                        // 3. 그리고 해당 2차포인터를 이용하여 1차포인터를 다른 객체가 저장된 1차포인터로 바꿈.
	                                                                        // 4. 생성한 객체는 해제하지 않고 무언가를 덮어쓰면 메모리 누수가 일어날 수 있음. 따라서 객체를 새로 생성하고 포인터만 바꿈.
	...
	// PS의 CB도 등록은 하고 있으나, 초기화 내용이 없음 (지금은 필요 X)
	...
	// - D3D11_CULL_MODE::D3D11_CULL_NONE 또는 삼각형 뒤집기					// D3D11_CULL_NONE : CULL을 하지 않음
	...
	// 중요! CubeMap은 안쪽이 전면이 되어야하므로 indices를 뒤집어서 앞뒤를 바꿔줘야함 (인덱스의 시작부터 끝까지 전부 뒤집음)
	std::reverse(cubeMeshData.indices.begin(), cubeMeshData.indices.end());

	// 큐브맵 버퍼 초기화 - m_cubeMapping 라는 구조체 내에 초기화 하고 있음
	...
	vector<D3D11_INPUT_ELEMENT_DESC> basicInputElements = {
	    ...
	    {"NORMAL", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 4 * 3,
	     D3D11_INPUT_PER_VERTEX_DATA, 0}, // 필요 업음. 그러나 InputLayOut 을 바꾸면 혼선이 올 수 있어서 없애지 않음
...
bool ExampleApp::Initialize()
{
	...
	// 매쉬들의 meshData.vertices를 newMesh->vertexBuffer에 저장
	// 매쉬들의 meshData.indices를 newMesh->indexBuffer에 저장
	for (const auto &meshData : meshes)
	{
		...
		{
			// meshData.textureFilename의 이름을 가진 텍스쳐를 리소스로 만들어 newMesh->texture에 저장
			// 해당 리소스를 쉐이더에서 읽어 사용할 수 있게 SRV를 만들어 newMesh->textureResourceView에 저장
...
void ExampleApp::Update(float dt)
{
	...
	// 기본
	// m_BasicVertexConstantBufferData 초기화
	// 모델의 변환
	...
	// 재질값 설정 (ambient, shininess는 초기값 X)
...
void ExampleApp::Render()
{
	...
	m_context->PSSetShaderResources(
	    0, 1, m_cubeMapping.cubemapResourceView.GetAddressOf());
