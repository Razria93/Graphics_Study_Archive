...
void AppBase::CreateDepthBuffers()
{
	...
	// DepthStencilView
	// DSV는 RTV와 같은 역할 계층 [출력 전용, Output-only]
	// - 그래픽스 파이프라인의 마지막 단계(Output-Merger, OM)에서
	//	 셰이더 연산 결과(픽셀 색상, 깊이값 등)를 GPU가 “기록(write)”하는 전용 뷰(View)
	// - 즉, 파이프라인에서 연산되어 나온 결과를 받아 쓰기 위한 뷰
	// - D24_S8, D32와 같은 포멧은 '깊이 비교 연산용'으로 예약되어 있어서 CreateDSV를 해야할 경우 해당 포멧을 사용해야함
	// - DSV에 관한 추가적인 정보는 옵시디언에서 참고 (Early-Z/Late-Z와 RTV/SRV의 읽기쓰기 관련)
	...
	ThrowIfFailed(m_device->CreateTexture2D(&desc,
	                                        0,
	                                        depthStencilBuffer.GetAddressOf()));

	// 일반적으로 사용하는 DepthStencilView 는 MSAA 사용여부에 따라 달라지겠지만
	// 일반적으로는 count = 4
	ThrowIfFailed(m_device->CreateDepthStencilView(depthStencilBuffer.Get(),
	                                               NULL, // NULL 일 경우 desc 설정값 사용
	                                               m_depthStencilView.GetAddressOf()));

	// Depth 전용
	// DepthMap을 만드는 이유는 다른 쉐이더에 넣어서 추가적인 효과를 구현하기 위한 것
	// 그런데 MultiSampling 정보를 가지고 있는 텍스쳐인 [Texture2DMS는 쉐이더의 리소스로 활용하기가 여러움]
	// 따라서 여기서는 Count = 1 인 Texture2D를 사용하고 있음
	// 
	// 그리고 여기 Formatdl R32_TYPELESS 인 부분도 주목해야함
	// DSV는 반드시 D32, D24_S8와 같은 형태의 포멧으로 구성해야함.
	// 그러나 m_depthOnlyBuffer의 경우 R32_TYPELESS
	// 그 이유는 해당 텍스쳐를 PS에서도 접근하여 활용하고 싶기 때문임
	// 따라서 변환이 가능하도록 TYPELESS로 설정해둔 뒤, DSV_DESC와 SRV_DESC를 이용하여 각각을 생성하는 것
	//
	// 또한 이를 별도로 만드는 이유는 MSAA가 적용된 Depth값을 Resolved 할 수 없음
	// ComputeShader를 통해서 수동으로 할 수는 있으나 그렇게까지 별도로 처리할 필요는 없다는 것
	desc.Format = DXGI_FORMAT_R32_TYPELESS;	// D24_S8 -> R32_TYPELESS
	desc.BindFlags = D3D11_BIND_DEPTH_STENCIL | D3D11_BIND_SHADER_RESOURCE; // SRV 용도 추가
	desc.SampleDesc.Count = 1;
	desc.SampleDesc.Quality = 0;
	ThrowIfFailed(m_device->CreateTexture2D(&desc,
	                                        NULL,	
	...
	// CreateDepthStencilView의 D3D11_DEPTH_STENCIL_VIEW_DESC 인자가
	// 1. NULL일 경우, TextureDesc 기반으로 Direct3D가 자동으로 일치하는 기본 뷰 생성
	// 2. NULL이 아닐 경우, 개발자가 지정한 Format / ViewDimension / MipSlice / ArraySlice 등을 반영해서 커스텀 뷰 생성
	
	// DSV를 만들 때는 D32 float (사용하는 포멧이 다름)
	D3D11_DEPTH_STENCIL_VIEW_DESC dsvDesc;
	ZeroMemory(&dsvDesc, sizeof(dsvDesc));
	dsvDesc.Format = DXGI_FORMAT_D32_FLOAT;
	dsvDesc.ViewDimension = D3D11_DSV_DIMENSION_TEXTURE2D;

	ThrowIfFailed(m_device->CreateDepthStencilView(m_depthOnlyBuffer.Get(),
	                                               &dsvDesc,
	                                               m_depthOnlyDSV.GetAddressOf()));

	// SRV를 만들 때는 R32 float (사용하는 포멧이 다름)
...
void AppBase::CreateBuffers()
{
	...
	// No MSAA 형식을 사용하는
	// m_postEffectsBuffer, m_resolvedSRV, m_resolvedRTV, m_postEffectsSRV,
	// m_postEffectsRTV
