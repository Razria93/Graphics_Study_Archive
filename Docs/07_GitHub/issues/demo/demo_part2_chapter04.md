...
`Part2_Chapter04`는 triangle coverage와 barycentric interpolation에서 시작한다.

흐름은 2D transform, depth test, culling, perspective interpolation과 CPU lighting으로 확장된다.

Step8 PerspectiveProjection을 Chapter 대표 결과로 둔다.

Step10 Lights는 기술 보조, Step1A Triangle To Circle은 Personal Extension으로 연결한다.

Step4 Animation2D는 시간 변화 사례로 함께 기록한다.
...
Segment를 32로 조정해 같은 반지름과 color interpolation을 유지한 결과를 확인한다.
...
Affine weight로 휘어진 checker가 reciprocal-depth 보정 뒤 안정된 형태로 나타난다.
...
Directional의 전역 방향, Point의 거리 감쇠, Spot의 cone 집중을 비교한다.
...
각 triangle의 screen-space bounding box를 순회한다.

Edge function으로 내부 pixel을 판정한다.

Barycentric weight로 vertex attribute를 보간한다.

이 기반은 이후 depth, texture coordinate와 lighting input까지 이어진다.
...
Raster space의 affine weight를 vertex의 eye-relative Z로 다시 계산한다.

이 보정은 color와 UV에 적용된다.
...
세 결과는 CPU shading 입력과 분기 차이가 밝기 분포에 미치는 영향을 보여준다.
...
- 포함: Step1부터 Step10까지의 triangle rasterization과 indexed geometry를 다룬다.
- 포함: 2D transform, animation, depth test를 같은 흐름에 둔다.
