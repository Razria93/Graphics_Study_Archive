...
void AppBase::CreateDepthBuffers()
{
	...
	for (int i = 0; i < MAX_LIGHTS; i++)
	{
		ThrowIfFailed(m_device->CreateDepthStencilView(m_shadowBuffers[i].Get(),
		                                               &dsvDesc,
	...
	ThrowIfFailed(m_device->CreateShaderResourceView(m_depthOnlyBuffer.Get(),
	                                                 &srvDesc,
	                                                 m_depthOnlySRV.GetAddressOf()));

	// 그림자 SRVs
	// 기록한 데이터를 Shader에서도 접근해서 사용할 수 있게 SRV 생성함
	for (int i = 0; i < MAX_LIGHTS; i++)
	{
		ThrowIfFailed(m_device->CreateShaderResourceView(m_shadowBuffers[i].Get(),
		                                                 &srvDesc,
...
bool AppBase::InitMainWindow()
{
	...
	m_mainWindow = CreateWindow(wc.lpszClassName, L"ComputerGraphics - Chapter13 Step4 ShadowMapping",
	                            WS_OVERLAPPEDWINDOW,
	                            100,                // 윈도우 좌측 상단의 x 좌표
	                            100,                // 윈도우 좌측 상단의 y 좌표
