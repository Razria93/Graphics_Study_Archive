...
bool AppBase::InitMainWindow()
{
	...
	m_mainWindow = CreateWindow(wc.lpszClassName, L"ComputerGraphics - Chapter11 Step5 HDRPipeline",
	                            WS_OVERLAPPEDWINDOW,
	                            100,                // 윈도우 좌측 상단의 x 좌표
	                            100,                // 윈도우 좌측 상단의 y 좌표
...
bool AppBase::InitDirect3D()
{
	...
	// 최종적으로 모니터에 출력될 백버퍼의 포멧은 DXGI_FORMAT_R8G8B8A8_UNORM
	...
	m_postProcess.Initialize(m_device,
	                         m_context,
	                         {m_resolvedSRV},   // 후처리에 기본SRV는 TEXTURE2DMS
	                         {m_backBufferRTV}, // 처리된 값을 백퍼에 넣음
	                         m_screenWidth,
	                         m_screenHeight,
	                         4);
...
void AppBase::CreateBuffers()
{
	...
	// swapChain의 backBuffer가 유효한지
	ThrowIfFailed(m_swapChain->GetBuffer(0,
	                                     IID_PPV_ARGS(backBuffer.GetAddressOf())));

	// backBuffer로 만든 RTV가 유효한지
	ThrowIfFailed(m_device->CreateRenderTargetView(backBuffer.Get(),
	                                               NULL,
	                                               m_backBufferRTV.GetAddressOf()));

	// FLOAT MSAA RenderTargetView/ShaderResourceView
	// 해당 그래픽카드에서 포멧, 샘플 포인트 수, 품질 레벨의 지원 여부르 묻는 함수
	// DXGI_FORMAT_R16G16B16A16_FLOAT가 되는지 묻고 있음
	ThrowIfFailed(m_device->CheckMultisampleQualityLevels(DXGI_FORMAT_R16G16B16A16_FLOAT,
	                                                      4,
	                                                      &m_numQualityLevels));
	...
	desc.BindFlags = D3D11_BIND_RENDER_TARGET | D3D11_BIND_SHADER_RESOURCE; // RTV, SRV 모두 생성가능
	desc.Format = DXGI_FORMAT_R16G16B16A16_FLOAT;                           // R16G16B16A16_FLOAT 포멧
	...
	{
		desc.SampleDesc.Count = 4;                        // 샘플 포인트 수 설정 (4개)
		desc.SampleDesc.Quality = m_numQualityLevels - 1; // 샘플 배치 패턴의 인덱스 (해당 샘플 포인트를 어떻게 배치할지를 정하는 인덱스)
	}
	else
	{
	...
	// [ViewDimension 결정 원리]
	// 1) CreateShaderResourceView()와 CreateRenderTargetView()는 Texture의 SampleDesc.Count 값에 따라 자동 설정됨
	//		- Count == 1  → D3D11_RTV_DIMENSION_TEXTURE2D
	//		- Count > 1   → D3D11_RTV_DIMENSION_TEXTURE2DMS (MSAA)
	// 2) 즉, m_useMSAA && m_numQualityLevels 조건에 따라 TEXTURE2D / TEXTURE2DMS가 자동 결정됨
	// 3) TEXTURE2DMS의 텍스쳐를 다른 곳의 ResourceView로 넘겨주기 위해서는 Resolve를 통해 평균화를 시켜야함
	...
	// 여기서는 TEXTURE2D
	// 따라서 다른 곳의 ResourceView로 넘겨줄 수 있음 (->PostProcess)
	desc.SampleDesc.Count = 1;
	desc.SampleDesc.Quality = 0;
