...
bool ExampleApp::Initialize() {
    ...
    */
    ...
    // Main Sphere
    {
        ...
        m_mainSphere.m_basicPixelConstantData.material.diffuse = Vector3(0.8f);
    ...
    // Main sphere와의 충돌이 감지되면 월드 공간에 작게 그려지는 구
    {
        ...
        m_cursorSphere.m_basicPixelConstantData.useNormalMap = false;
        m_cursorSphere.m_basicPixelConstantData.useAOMap = false;
        m_cursorSphere.m_basicVertexConstantData.useHeightMap = false;
    ...
    // Ground
    {
        ...
        ground.albedoTextureFilename =
            ...
            "Bricks075A_1K-PNG_Color.png";

        ground.normalTextureFilename =
            "../Assets/Textures/PBR/Bricks075A_1K-PNG/"
            "Bricks075A_1K-PNG_NormalDX.png";

        ground.heightTextureFilename =
            "../Assets/Textures/PBR/Bricks075A_1K-PNG/"
            "Bricks075A_1K-PNG_Displacement.png";

        ground.aoTextureFilename = "../Assets/Textures/PBR/Bricks075A_1K-PNG/"
                                   "Bricks075A_1K-PNG_AmbientOcclusion.png";
...
void ExampleApp::Update(float dt) {
    ...
    // 포인트 라이트 효과
    m_meshGroupGround.m_basicPixelConstantData.lights[1].position =
        m_lightPosition;
    m_meshGroupGround.m_basicPixelConstantData.lights[1].strength =
        Vector3(1.0f);
    m_meshGroupGround.m_basicPixelConstantData.lights[1].fallOffEnd = 10.0f;
    m_meshGroupGround.m_basicPixelConstantData.lights[0].strength =
        Vector3(0.0f);
    m_meshGroupGround.m_basicPixelConstantData.lights[2].strength =
        Vector3(0.0f);
...
void ExampleApp::UpdateGUI() {
    ...
    //                    &m_mainSphere.m_basicPixelConstantData.mipmapLevel,
    //                    0.0f, 10.0f);
    ...
    */
    ...
    bool temp2 = bool(m_meshGroupGround.m_basicPixelConstantData.useTexture);
    if (ImGui::Checkbox("Use AlbedoTexture2", &temp2)) {
        m_meshGroupGround.m_basicPixelConstantData.useTexture = int(temp2);
    }

    temp2 = bool(m_meshGroupGround.m_basicPixelConstantData.useNormalMap);
    if (ImGui::Checkbox("Use NormalMapping2", &temp2)) {
        m_meshGroupGround.m_basicPixelConstantData.useNormalMap = int(temp2);
    }

    temp2 = bool(m_meshGroupGround.m_basicPixelConstantData.useAOMap);
    if (ImGui::Checkbox("Use AO2", &temp2)) {
        m_meshGroupGround.m_basicPixelConstantData.useAOMap = int(temp2);
    }

    temp2 = bool(m_meshGroupGround.m_basicVertexConstantData.useHeightMap);
    if (ImGui::Checkbox("Use HeightMapping2", &temp2)) {
        m_meshGroupGround.m_basicVertexConstantData.useHeightMap = int(temp2);
    }
