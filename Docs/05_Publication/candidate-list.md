...
| `02_Bloom` Demo publication bundle | `Docs/03_Demos/Part1_Chapter01-02/02_Bloom.md`, `Docs/07_GitHub/issues/demo/demo_part1_chapter01-02.md`, `Docs/_assets/captures` | input/result와 구현 선택을 독립적인 시각·기술 설명으로 제공 가능 | 검토 필요 | 육안 검사에서 워터마크, 개인 식별자, application UI가 보이지 않음. 입력 이미지는 ChatGPT 생성 asset으로 기록됨 | 파일 metadata 검사, 생성 asset provenance 문구 확정, commit permalink 확정 | Demo Issue 게시와 public 공개 확정은 별도 승인 대상 |
