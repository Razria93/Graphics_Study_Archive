...
float4 main(PixelShaderInput input) : SV_TARGET
{
    ...
    float theta = atan2(input.posModel.z, input.posModel.x);
    float phi = acos(input.posModel.y / radius);
    
    texcoord.x = theta / (3.141592 * 2.0f);
    texcoord.y = phi / (3.141592 * 1.0f);
    
    return useTexture ? float4(color, 1.0) * g_texture0.Sample(g_sampler, texcoord) : float4(color, 1.0);
}
