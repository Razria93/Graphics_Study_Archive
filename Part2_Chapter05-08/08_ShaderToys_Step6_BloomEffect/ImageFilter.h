...
class ImageFilter
{
  public:
	                ...
	                int width, int height)
	{
		...
		// 자체적으로 텍스쳐를 가지며, 여기에 쓰기할 수 있는 RTV와 읽기할 수 있는 SRV를 생성
	...
	// 해당 과정은 VS에서 딱히 처리할 것이 없고 PS에서 처리되기 때문에 VS의 CB는 업데이트 X
	void UpdateConstantBuffers(ComPtr<ID3D11Device> &device,
	                           ComPtr<ID3D11DeviceContext> &context)
	{
		// 픽셀의 해상도가 달라져 단위 픽셀정보가 달라질 수 있음
		// 달라진 m_pixelConstData 내용을 CB에 반영함
