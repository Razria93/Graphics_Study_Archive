          ...
          uint3 dtID : SV_DispatchThreadID)
{
    ...
    // MyCode
    // float2 eta = float2(abs(vorticity[right]) - abs(vorticity[left]), abs(vorticity[up]) - abs(vorticity[down]));
    // eta *= 0.5;
    // 
    // if (length(eta) < 1e-5)
    //     return;
    // 
    // float3 psi = float3(eta.x / length(eta), eta.y / length(eta), 0.0);
    // float3 omega = float3(0.0, 0.0, vorticity[dtID.xy]);
    // 
    // velocity[dtID.xy] += cross(psi, omega);
    ...
    // c는 작은 회전의 정도
    float c = 1.0;
    velocity[dtID.xy] += c * cross(psi, omega).xy * dx;
}
