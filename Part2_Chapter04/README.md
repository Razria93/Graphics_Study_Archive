...
- 순차 문서화: Step1 Triangle 진행 중
- Build/run: Step1 현재 확인, 나머지 예제 과거 확인
- Capture: Step1 전체 창 screenshot 확보
...
| Next action | Step1 마감 후 Personal Extension 정규화 |

## 학습 순서

1. [Step1 Triangle](04_Rasterization_Step1_Triangle/README.md)
2. Personal Extension: Triangle To Circle
3. Step2 Circle
...
`04_Rasterization_Step1_TriangleToCircle`은 강의 단계가 아니라 사용자가 Step1 triangle rasterization을 triangle fan 기반 circle approximation으로 확장한 Personal Extension이다. Step2와 합치지 않고 독립 Example과 상세 Demo로 정규화하며, 현재는 code 보관과 과거 build/run 확인 상태만 유지한다.
...
| Example | 책임 | 현재 상태 |
| --- | --- | --- |
| `04_Rasterization_Step1_TriangleToCircle` | Step1 triangle fan 응용 | Personal Extension, 정규화 대기 |
