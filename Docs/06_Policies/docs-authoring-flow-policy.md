...
- 일반 본문은 80자에 맞춰 기계적으로 줄바꿈하지 않는다. 문장과 의미 단위를 우선하고, 긴 문장은 필요한 경우 문장 자체를 나눈다.
...
- URL, image Markdown, table, 긴 경로처럼 분할하면 의미나 동작이 깨지는 항목은 일반 본문 길이 검사에서 제외한다.
- validator는 물리적 길이와 형식을 검사한다. 링크와 조사·서술어의 의미상 결합, 문장 흐름, 렌더링 가독성은 agent 또는 수동 검수에서 확인한다.
