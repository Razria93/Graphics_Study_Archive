...
namespace hlab
{
	...
	{
		triangle.v0.pos = { 0.0f, 1.0f, 1.0f };
		triangle.v1.pos = { -1.0f, -1.0f, 1.0f };
		triangle.v2.pos = { 1.0f, -1.0f, 1.0f };
		
		triangle.v0.color = { 1.0f, 0.0f, 0.0f };
		triangle.v1.color = { 0.0f, 1.0f, 0.0f };
		triangle.v2.color = { 0.0f, 0.0f, 1.0f };
	}
	...
	{
		const vec2 Rast_v0 = ProjectWorldToRaster(triangle.v0.pos);
		const vec2 Rast_v1 = ProjectWorldToRaster(triangle.v1.pos);
		const vec2 Rast_v2 = ProjectWorldToRaster(triangle.v2.pos);
		...
		for (size_t j = yMin; j <= yMax; j++)
		{
			for (size_t i = xMin; i <= xMax; i++)
			{
				...
				if (alpha0 >= 0 && alpha1 >= 0 && alpha2 >= 0)
				{
					...
					const float color_R = 
						alpha0 * triangle.v0.color.r +
						alpha1 * triangle.v1.color.r +
						alpha2 * triangle.v2.color.r;

					const float color_G = 
						alpha0 * triangle.v0.color.g +
						alpha1 * triangle.v1.color.g +
						alpha2 * triangle.v2.color.g;

					const float color_B = 
						alpha0 * triangle.v0.color.b +
