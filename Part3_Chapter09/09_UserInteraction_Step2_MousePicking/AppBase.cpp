	...
	while (WM_QUIT != msg.message)
	{
		...
		else
		{
			...
			if (m_captureFlag)
			{
				...
				// 멀티샘플링을 해서 결과가 저장되어 있는 Texture2DMS를 일반적으로 사용하는 Texture2D로 변환하는 과정

				// Swapchain에서 쓰는 버퍼를 가져옴
				ComPtr<ID3D11Texture2D> backBuffer;
				m_swapChain->GetBuffer(0,
				                       IID_PPV_ARGS(backBuffer.GetAddressOf()));

				// 해당 결과를 m_tempTexture로 저장함
				// GPU(백버퍼) -> GPU(Resource)
				m_context->ResolveSubresource(m_tempTexture.Get(), 0,
				                              backBuffer.Get(), 0,
				                              DXGI_FORMAT_R8G8B8A8_UNORM);

				// 백버퍼를 직접 하지 않는 이유
				// 1. Swapchain에서 MultiSampling을 사용하고 있기 때문에 png로 변환가능한 일반 포멧이 아님
				// MultiSampling이 뭔지 찾아보자
				// TextureMemory의 용도를 명확하게 지정해줘야함 (GPU의 효율을 높이기위해)
				// 그래서 Desc에서 CPU에서 접근하게 만들어준 desc으로 만든 텍스쳐만 CPU 접근 가능
...
bool AppBase::CreateRenderTargetView()
{
	...
	if (backBuffer)
	{
		// backbuffer의 메모리 공간에 GPU가 값을 변경 할 수 있게 해주는
		// backbuffer 대상의 RTV 생성
		...
		desc.BindFlags = D3D11_BIND_SHADER_RESOURCE; // 용도는 SRV (읽기)
		desc.MiscFlags = 0;

		// Filter용 텍스쳐
		// SRV(읽기)으로만 사용 가능
		// 그저 backbuffer와 설정이 같은 Texture2D(버퍼) 생성
		...
		// SRV(읽기)으로만 사용 가능
		// 그저 backbuffer와 설정이 같은 Texture2D(버퍼) 생성
		// 그러나 해당 텍스처는 기록(Render)이 될 수 없기 때문에 복사(Copy)로 데이터를 채워야함
		// 보통 특정 버퍼를 바이트 단위로 Copy함 (Map / UnMap)
		...
		// Filter용 SRV [얘는 RTV X]
		// SRV 용으로 생성한 텍스쳐에 읽기를 할 수 있는 COM객체인 SRV를 생성
		// 그러나 해당 텍스처는 기록(Render)이 될 수 없기 때문에 복사(Copy)로 데이터를 채워야함
		// 보통 특정 버퍼를 바이트 단위로 Copy함 (Map / UnMap)
		m_device->CreateShaderResourceView(m_tempTexture.Get(), nullptr,
		                                   m_shaderResourceView.GetAddressOf());

		// Picking용으로 사용하기 위해 추가 설정
		// Staging -> CPU <-> GPU 간 데이터 전송용 임시버퍼
		// D3D11_CPU_ACCESS_READ 이기 때문에 GPU Write / CPU Read
		// D3D11_CPU_ACCESS_Write 였으면 GPU Read / CPU Write
		// 1x1 작은 스테이징 텍스춰 만들기
		desc.BindFlags = 0;
