          ...
          uint3 dtID : SV_DispatchThreadID)
{
    ...
    // TODO: vorticity °è»ê
    // MyCode
    // float curl = (velocity[right].y - velocity[left].y) - (velocity[up].x - velocity[down].x);
    // vorticity[dtID.xy] = curl * 0.5;

    // ExampleCode
    vorticity[dtID.xy] = float(((velocity[right].y - velocity[left].y) / (2.0 * dx.x))
