...
- Build/run status: Step1, TriangleToCircle, Step2, Step3, Step4 성공 / Step5 빌드 성공, 실행 미확인 / 이후 미확인
...
| `04_Rasterization_Step5_DepthBuffer` | 반영 완료 | 빌드 성공 / 실행 미확인 | 검토 필요 | depth buffer visibility test |
...
1. `04_Rasterization_Step5_DepthBuffer` Debug/Release 실행 확인이 필요합니다.
2. 확인 전까지 Step5 run status는 `미확인`으로 유지합니다.
