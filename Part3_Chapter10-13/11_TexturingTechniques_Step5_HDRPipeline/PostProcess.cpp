    ...
    const int height, const int bloomLevels)
{
	...
	// 한번 만든 Square에 대한 VB, IB를 계속 재활용함
	// 즉, PostProcess 객체를 하나 만들어서 쉐이더만 변경해서 계속 쓰는듯
	...
	for (int i = 0; i < bloomLevels - 1; i++)
	{
		...
		m_bloomDownFilters[i].Initialize(device, context, m_bloomDownPixelShader, width / div, height / div);
		if (i == 0)
		{
...
void PostProcess::Render(ComPtr<ID3D11DeviceContext>& context)
{
	...
	context->IASetVertexBuffers(0, 1, m_mesh->vertexBuffer.GetAddressOf(),
	                            &stride, &offset);
	context->IASetIndexBuffer(m_mesh->indexBuffer.Get(), DXGI_FORMAT_R32_UINT, // TODO
	                          0);
                               ...
                               ComPtr<ID3D11RenderTargetView>& rtv)
{
	...
	// ThrowIfFailed를 체크하는 것은 중요한 작업
	// ThrowIfFailed 내부에 브레이크 포인트를 걸어두면 문제가 생기는 경우 거기에 걸리면서 결과값을 확인할 수 있음
