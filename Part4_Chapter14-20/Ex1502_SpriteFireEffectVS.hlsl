...
PSInput main(uint vertexID : SV_VertexID)
{
    ...
    // float3 color1 = float3(0.0f, 0.0f, 1.0f);
    float3 color1 = float3(1.0f, 0.5f, 0.2f);
    float3 color2 = float3(1.0f, 0.3f, 0.0f);
    
    float3 tempColor = lerp(color2, color1, pow(saturate(p.life / 1.0f), 2.0)); // life에 따라서 color가 달라짐
