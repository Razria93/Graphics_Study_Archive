...
void ExampleApp::Render()
{
	...
	// 해당 PSO에 의해 m_resolvedRTV는 (1,1,1,1) 만 기록됨
	// 의도적으로 Depth만 기록하기 위함
	...
	// PostEffects
	// 이미지 필터를 PSO를 기반으로 밖으로 끄집어낸 것
	AppBase::SetPipelineState(Graphics::postEffectsPSO);
	vector<ID3D11ShaderResourceView*> postEffectsSRVs = {m_resolvedSRV.Get(),   // 기존 패스 렌더링 작업물 SRV
	                                                     m_depthOnlySRV.Get()}; // 뎁스 텍스쳐 SRV
	m_context->PSSetShaderResources(20,                                         // 20번에 넣어줌
	                                UINT(postEffectsSRVs.size()),
	                                postEffectsSRVs.data());

	m_context->OMSetRenderTargets(1, m_postEffectsRTV.GetAddressOf(), NULL);

	m_context->PSSetConstantBuffers(3,
	                                1,
