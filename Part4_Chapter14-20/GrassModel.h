...
class GrassModel : public Model
{
  ...
  public:
	                ...
	                ComPtr<ID3D11DeviceContext>& context)
	{
		...
		// 잔디는 그림자맵 만들때 제외 (이미지 기반 자체 그림자 효과 구현)
		m_castShadow = false;
		...
		D3D11Utils::CreateInstanceBuffer(device, m_instancesCpu,	// Instance : 10만개 제작
		                                 m_instancesGpu);
	...
	void Render(ComPtr<ID3D11DeviceContext>& context) override
	{
		if (m_isVisible)
		{
			...
			// VertexBuffer 가 2개
			// GrassVertex 는 1개, GrassInstance가 여러개
			context->IASetVertexBuffers(0, 2, vertexBuffers, strides, offsets);
			context->IASetIndexBuffer(m_indexBuffer.Get(), DXGI_FORMAT_R32_UINT,
			                          0);
			context->IASetInputLayout(Graphics::grassIL.Get());					// 중요!
			context->DrawIndexedInstanced(m_indexCount, m_instanceCount, 0, 0,	// m_indexCount			: 1개의 모델이 Vertex 몇개인지
			                              0);									// m_instanceCount		: 총 몇개 그릴 것인지
																				// DrawIndexedInstanced : Instancing을 사용할 때 사용하는 DrawCall
		}
	};
