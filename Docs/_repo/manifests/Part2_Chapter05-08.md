...
- Build/run status: Step1 Matrix(GLM), Step2 Lights(GLM) 성공 / Step3 DirectXMath 빌드 성공, 실행 미확인
...
| `05_AffineTransformations_Step3_DirectXMath` | 반영 완료 | 빌드 성공 / 실행 미확인 | 검토 필요 | DirectXMath load/store, SimpleMath comparison |
...
1. `05_AffineTransformations_Step3_DirectXMath` Debug/Release 실행을 확인합니다.
2. 사용자가 Debug/Release 실행 확인을 완료하면 status 문서와 Part2 README를 갱신합니다.
3. 다음 후보는 `05_AffineTransformations_Step4_Lights(SimpleMath)`입니다.
