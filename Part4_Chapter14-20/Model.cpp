	...
	for (const auto& meshData : meshes)
	{
		...
		if (!meshData.albedoTextureFilename.empty())
		{
			if (filesystem::exists(meshData.albedoTextureFilename))
			{
				...
				{
					// opacityTextureFilename가 있을 경우 RGB값을 저장하고 
					// float3(3채널) 배열 albedo(RGB)와 float배열(1채널) opacity(Alpha)를 합쳐서 float4(4채널) texture를 만들고 SRV를 만듦
...
void Model::Render(ComPtr<ID3D11DeviceContext>& context)
{
	if (m_isVisible)
	{
		for (const auto& mesh : m_meshes)
		{
			...
			vector<ID3D11ShaderResourceView*> resViews = {mesh->albedoSRV.Get(),
			                                              mesh->normalSRV.Get(),
			                                              mesh->aoSRV.Get(),
			...
			// Volume Rendering
			// Render 자체는 볼륨인지 상관없이 계속 호출되나, 내외부적으로 등록하고 호출하는 식
