# Status: 10_GeometryPipeline_Step3_NormalLines
...
Primary raw folder `C:\ComputerGraphics\Part3_Chapter10_13\10_GeometryPipeline_Step3_NormalLines`를 기준으로 import했습니다.
...
- `NormalPixelShader.hlsl` and `NormalVertexShader.hlsl` had incomplete Release x64 shader settings. Added Release x64 `ShaderType`/`ShaderModel 5.0`.
- `NormalGeometryShader.hlsl` already had Release x64 Geometry/4.0 settings in raw and was kept as-is.
...
- Selected asset reuse check: 통과, Step1/Step2 shared assets 재사용

## Follow-up

- 다음 import 대상은 `10_GeometryPipeline_Step4_Fireball`입니다.
