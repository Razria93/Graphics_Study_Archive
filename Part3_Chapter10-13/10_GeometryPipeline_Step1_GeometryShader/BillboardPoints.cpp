...
void BillboardPoints::Render(ComPtr<ID3D11DeviceContext> &context) {
    ...
    // HERE : 바꿔줄 필요 없이 그대로 POINTLIST 사용
    // context->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

    // HERE : POINTLIST는 연결관계가 필요 없기 때문에 DrawIndexed() 대신 Draw() 사용
    context->Draw(m_indexCount, 0);

    // HERE : Geometry Shader를 쓰지 않는 다른 물체들을 위해 nullptr로 설정
