...
| `02_Bloom` | `Part1_Chapter01-02/02_Bloom/02_Bloom.sln` | 확인 | 확인 | 확인 | 확인 | 보류 | Debug/Release x64 build/run 확인. `result.png` 생성 확인. 결과 이미지는 워터마크/권리 검토 전까지 tracked capture/result로 승격하지 않음 |
...
| Capture/Result | 보류 | 결과 이미지에 강의/출처 표기가 포함되어 public capture/result 승격 전 권리 검토 필요 |
