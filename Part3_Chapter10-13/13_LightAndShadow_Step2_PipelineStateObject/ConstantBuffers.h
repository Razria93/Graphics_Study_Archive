...
{
	// 행렬이름을 modelWorld -> world로 변경
	Matrix world;	// DX, UE5 모두 world 채택 (model의 리소스를 world에 맞게 변환하는 행렬)
	Matrix worldIT; // invTranspose
	int useHeightMap = 0;
...
__declspec(align(256)) struct MaterialConstants
{
	...
	// HLSL은 bool이 4byte
	// 따라서 HLSL에서는 일반적으로 bitMask를 활용함
	// 여기서는 일단 int로 설정
...
{
	// view proj를 나눠둔 이유?
	// Skybox 에서는 view에서 회전만 적용시켜야하므로 별도로 저장
	// 이외에도 여기저기서 쓰임
	Matrix view;
	Matrix proj;
	Matrix viewProj;
	...
	// light는 한 물체를 렌더링할 때 여러개가 사용될 수 있기 때문에 구조체 배열형태
	// 또한 lights는 Scene 내부의 모든 obj 렌더링에 영향을 미칠 수 있으므로 GlobalConstants
