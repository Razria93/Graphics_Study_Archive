                                ...
                                const std::vector<MeshData>& meshes)
{
	...
	// = Init NormalVertexBuffer
	//	* - DrawNormal 용도의 VertexBuffer를 만들기 위해 여러 메쉬의 Vertices를 하나로 합치는 작업
	size_t offset = 0;
	for (const auto& meshData : meshes)
	{
		for (size_t i = 0; i < meshData.vertices.size(); i++)
		{
			m_normalVertices.push_back(meshData.vertices[i]);
		}
		offset += meshData.vertices.size();
	}

	D3D11Utils::CreateVertexBuffer(device,
	                               m_normalVertices,
	                               m_normalVertexBuffer);

	 m_indexCount = uint32_t(m_normalVertices.size());
...
void BasicMeshGroup::Render(ComPtr<ID3D11DeviceContext>& context)
{
	...
	if (m_drawNormals)
	{
		...
		context->IASetVertexBuffers(0, 1,
		                            m_normalVertexBuffer.GetAddressOf(),
		                            &stride,
		                            &offset);
