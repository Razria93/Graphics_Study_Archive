...
| Sprite fire particles | [Ex1502 SpriteFireEffect](15_02_SpriteFireEffect.md) | `Ex1502_SpriteFireEffect` | GPU Simulation, Texturing | [Verification](../../02_Verification/Part4_Chapter14-20/verification-index.md) | local screenshot 후보 | [local 후보](../../07_GitHub/issues/demo/demo_part4_chapter15.md) | 후보 | 원본 `flare0.dds`를 직접 링크하지 않고 rendered evidence 후보로만 다룬다. public 대표 visual은 provenance 확인 또는 대체 asset 검토 후 판단 |
...
- Chapter15 `Ex1501`과 10초 지연 `Ex1503` selected screenshot은 `Docs/_assets/captures`에 승격하고 상세 Demo와 local Demo Issue 후보에 연결했다. `Ex1502`는 상세 Demo로 구현 흐름을 기록하되 원본 `flare0.dds`를 직접 링크하지 않고 rendered evidence 후보로만 다룬다.
