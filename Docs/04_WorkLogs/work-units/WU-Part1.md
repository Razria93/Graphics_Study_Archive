...
| Chapter03 순차 작업 | 진행 중 | Step4 DrawingSphere부터 Step10 Texturing까지 build/run과 README·Topic을 완료했다. Step10 capture는 외부 입력 texture 권리 검토로 보류한다. |
...
1. Step10 input texture의 출처·라이선스를 확인하거나 승인된 asset으로 교체한다.
2. `Part1_Chapter03` Step11 Supersampling을 build/run하고 문서화한다.
3. Step12~14를 순서대로 검증하고 독립 상세 Demo를 축적한다.
